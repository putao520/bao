...
9. **会话互斥改物理性保障(2026-09-03 用户裁决适配:file_lock MCP 工具已随 gsc DEC-GSC-WORKSPACE-TOPOLOGY-1 W3b 退役,不再要求调用)**:互斥由 (a) launcher `flock` 单飞(防多 daily-ops 并发,`scripts/daily-ops.sh` 已有)+ (b) 开工前预检 `git status`——脏且脏文件集 ⊆ `pending_wave` 声明域 → 按 SKILL.md §4 暴毙中途态恢复继续;脏且含声明域外文件(交互会话在写)→ 本轮让位只读(SKILL.md §2 边界表 SKIPPED_BUSY 语义照旧);净树即本会话为唯一写者,正常执行写操作。MCP 工具面健康仍由 launcher 探针保证(探针目标已改 commit_gate,见 `scripts/daily-ops.sh`);会话启动时 MCP 工具面整体缺失(探针失败未挂载)→ 仍 fail-closed 本轮只读并登记报告升级(防环境假绿,保留)。收尾不再调 release(锁工具已无),改为确认 `git status` 与本轮预期写域一致(含声明域外残留 → 登记升级)
