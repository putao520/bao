...
impl CompileOptionsWrapper {
    ...
    /// Set the `hideScriptFromDebugger_` flag on the underlying CompileOptions.
    ///
    /// When `true`, the compiled script will NOT trigger `DebugAPI::onNewScript`,
    /// avoiding the `RememberSourceURL` path that walks the Zone's
    /// `AtomCacheHashTable`. BAO PATCH (BCE-20260622-004): used by bao's
    /// Node Realm evaluator to suppress the SIGSEGV-prone `onNewScript` hook
    /// that fires on every new script compilation and walks a cache that can
    /// contain stale atom entries (chars pointing to freed JSString memory)
    /// when multiple Realms/Zone have been created and destroyed across
    /// page lifecycles. See `src/bao_browser/src/runtime_bridge.rs::evaluate_in_node_realm`.
    pub fn set_hide_script_from_debugger(&mut self, hide: bool) {
        unsafe {
            // SAFETY: CompileOptionsWrapper.ptr points to an OwningCompileOptions
            // (subclass of ReadOnlyCompileOptions, which embeds TransitiveCompileOptions
            // where hideScriptFromDebugger_ lives as a POD bool). The struct is
            // __attribute__((packed)) so field offsets are deterministic.
            // ReadOnlyCompileOptions._base is TransitiveCompileOptions.
            (*self.ptr)._base.hideScriptFromDebugger_ = hide;
        }
    }
}
