...
SCRATCH = "mozjs-sys/etc/.update-scratch"
                ...
                with open(milestone_path) as f:
                    for line in f:
                        ...
                        if line and not line.startswith("#"):
                            # BAO DELTA: upstream appends ".0" unless the
                            # milestone has 3 dots — that yields an invalid
                            # 4-component semver for 3-component milestones
                            # like 153.3.0 ("153.3.0.0"). Cargo rejects those.
                            # Normalize to the milestone itself when it is
                            # already a valid x.y[.z] semver.
                            if line.count(".") < 2:
                                line += ".0"
                            return line
            return None
...
"""
        ...
        # BAO DELTA (#42 split): install through the satellite split instead
        # of a flat TARGET rsync. The filtered tree lands in a scratch dir,
        # then: core skeleton -> mozjs-sys/mozjs (minus satellite slices),
        # js -> src-js, intl + config/external/icu/data -> src-intl,
        # python + third_party/python -> src-python. Patches apply to the
...
def apply_patches():
    ...
    for p in patches:
        ...
        subprocess.check_call(
            ["git", "apply", "--reject", "--directory=" + SCRATCH, p],
            stdout=subprocess.DEVNULL,
        )
...
SATELLITE_ROOTS = {
    "src-js": ["js"],
    "src-intl": ["intl", "config/external/icu/data"],
    "src-python": ["python", "third_party/python"],
}
SPLIT_EXCLUDES = [
    "--exclude=/js",
    "--exclude=/intl",
    "--exclude=/python",
    "--exclude=/config/external/icu/data",
    "--exclude=/third_party/python",
]


def mirror_dir(from_path, to_path):
    """Hardlink farm: real directories, files hardlinked (copy cross-fs)."""
    ...
    for entry in os.listdir(from_path):
        ...
        if os.path.isdir(src):
            mirror_dir(src, dst)
        elif os.path.islink(src) or not os.access(src, os.R_OK):
...
def split_install():
    """BAO DELTA: distribute the patched scratch tree into the #42 split
    faces (core skeleton + three satellite source crates). Wholesale slice
    replacement — 140-era residuals in the slices die here."""
    print("Installing split faces.")
    subprocess.check_call(
        ["rsync", "--delete-excluded", *SPLIT_EXCLUDES, "--prune-empty-dirs",
         "--quiet", "--recursive", SCRATCH + "/", TARGET + "/"]
    )
    for sat, slices in SATELLITE_ROOTS.items():
        for sl in slices:
            dest = os.path.join(sat, "mozjs", sl)
            if os.path.exists(dest):
                shutil.rmtree(dest)
            mirror_dir(os.path.join(SCRATCH, sl), dest)
...
def main(args):
    ...
    for arg in args:
        if arg == "--no-patch":
            ...
            extract = arg
    if extract:
        extract_tarball(os.path.abspath(extract), commit=False)
    if patch:
        apply_patches()
    if extract:
        split_install()
    if commit and extract:
        faces = [TARGET, "src-js", "src-intl", "src-python",
                 "mozjs-extracted-crates", "mozjs-sys/etc"]
        subprocess.check_call(["git", "add", "--all", *faces],
                              stdout=subprocess.DEVNULL)
        subprocess.check_call(["git", "commit", "-s", "-m",
                               "Update SpiderMonkey"],
                              stdout=subprocess.DEVNULL)
