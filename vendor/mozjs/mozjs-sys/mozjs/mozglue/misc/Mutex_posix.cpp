...
mozilla::detail::MutexImpl::~MutexImpl() {
  // Bao patch: during C++ atexit/TLS teardown, the TLS backing memory for
  // static objects may already be unmapped. Accessing `this` (which lives in
  // TLS) causes SIGSEGV. Additionally, libtest thread-pool threads may still
  // hold the mutex → pthread_mutex_destroy returns EBUSY → original code
  // MOZ_CRASH. Since this only fires during process exit, skip all work.
  // The OS reclaims mutex resources regardless.
  return;
}
