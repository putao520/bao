...
// SM153: microtasks live in the ENGINE-owned job queue. Servo's own
// microtask runnables are enqueued as private-value entries via
// `JS::EnqueueMicroTask`; promise reaction jobs are enqueued by the engine
// itself. The checkpoint below drains both kinds (mirroring
// CycleCollectedJSContext), so the JobQueue "enqueuePromiseJob"/"empty"
...
use js::jsapi::{
    GetPromiseUserInputEventHandlingState, JS, JSTracer, PromiseUserInputEventHandlingState,
};
...
impl MicrotaskQueue {
    ...
    pub(crate) fn checkpoint(&self, cx: &mut JSContext, globalscopes: Vec<DomRoot<GlobalScope>>) {
        ...
        while unsafe { HasAnyMicroTasks(cx) } {
            ...
            if !unsafe { JS::IsJSMicroTask(generic_task.as_ptr()) } {
            ...
            js_micro_task.set(unsafe { JS::ToMaybeWrappedJSMicroTask(generic_task.as_ptr()) });
            execution_global
                .set(unsafe { JS::GetExecutionGlobalFromJSMicroTask(js_micro_task.get()) });
            if !unsafe {
            ...
            let interaction = if let Some(promise) =
                NonNull::new(unsafe { JS::MaybeGetPromiseFromJSMicroTask(js_micro_task.get()) })
            {
