...
/// so embedders can register host functions that are available to the evaluated script.
///
/// Usage: `register_embedder_callback(webview_id, |cx, global| { ... })` before calling
/// `WebView::evaluate_javascript`. The callback runs on the script thread with valid
/// JSContext and the page's Window global object.
...
/// Register a callback to be executed on the script thread when the next
/// `evaluate_javascript` call processes for the given WebView.
///
/// The callback receives `(cx: *mut JSContext, global: *mut JSObject)`.
/// It is the embedder's responsibility to cast these to the correct types.
...
/// Drain all pending callbacks for the given WebView.
...
impl ScriptThread {
    ...
    fn load(
        ...
        if !opts::get().disable_script_debugger {
            self.debugger_global
        ...
        for callback in drain_embedder_callbacks(webview_id) {
            unsafe {
                callback(
                    ...
                    global_scope.reflector().get_jsobject().get() as *mut c_void,
                );
            }
        }
