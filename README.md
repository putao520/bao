...
to be practical on hosted CI runners. GitHub Actions is therefore **disabled**;
the [`justfile`](./justfile) is the authoritative "does it pass" entry point
(`just` is required; all cargo steps force `--jobs 1` per the mozjs EBUSY patch).

```bash
just ci          # fmt + lint + check + test + bce (full local CI)
...
`.github/workflows/` is kept as documentation of *what* CI would run; `act`
(`just gha-*`) can reproduce it in a local container if you want, but the
hosted pipeline is intentionally off.
