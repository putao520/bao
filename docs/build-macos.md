...
| 2 | mozjs compile pass-ability on mac: `should_build_from_source() -> true` hardcode (`vendor/mozjs/mozjs-sys/build.rs:715`) + `fix_stale_archive_objects()` make patch never ran under mac clang / Xcode CLT (`CLANG_PATH`) | **mac** | open — M4 falsification target (§4 M4). 2026-09-10 probe note (#37): every apple branch reachable from a Linux cross build was verified live (CXXFLAGS `-stdlib=libc++` injection, `cpp_set_stdlib c++`, `rustc-link-lib=c++`, vsrc synthesis, virtualenv, configure fail-closed on the macOS SDK via `MACOS_SDK_PATH`/`--with-macos-sdk`); the remaining unknown is strictly the SM C++ compile+link under real mac clang/SDK — the build.rs apple arms themselves are no longer unverified |
