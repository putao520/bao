...
| `bun_uws_sys` | **Supported** — `LIBUS_USE_EPOLL` | Compile surface ready — `LIBUS_USE_KQUEUE`, darwin root certs | **Blocked at the usockets absorb** — the `LIBUS_USE_LIBUV` arm is wired end-to-end in `build.rs` (eventing/libuv.c over the vendored libuv supply, clang-cl flag set, windows system libs, vendored zlib/libdeflate faces) but refuses with an actionable message until the usockets tree absorbs oven-sh/bun `4af1842c8c` — the pre-absorb `eventing/libuv.c` cannot compile against its own header, and the same absorb drags the crypto/TLS signature evolution (`us_socket_adopt_tls`, 4-arg SNI resume) ([#34](https://github.com/putao520/bao/issues/34)) | narrowed build-script gate before any C compile |
...
| [#34](https://github.com/putao520/bao/issues/34) | `uv_*` symbol closure for the uWS windows arm | **Half-closed** — supply landed (`bun_libuv_sys` compiles the vendored oven-sh/libuv fork; 9595dbae); consumer arm wired but gated on the usockets@`4af1842c8c` absorb (drag the crypto/TLS signature evolution) |
| [#35](https://github.com/putao520/bao/issues/35) | `bao_uloop` IOCP arm | **Gate lifted** — windows routes to the absorbed `LIBUS_USE_LIBUV` backend + `bun_io::windows_event_loop` FilePoll; link-time closure pending #34's usockets absorb |
