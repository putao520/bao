...
fn test_response_id_propagation_positive() {
    ...
    assert!(resp.result.is_some());
    assert!(resp.error.is_none(), "successful command must not carry error");
}
...
fn test_unknown_command_error_code_is_method_not_found() {
    ...
    assert_eq!(err.code, -32601, "method-not-found MUST be -32601 (JSON-RPC 2.0)");
    assert!(err.message.contains("Page.nonexistentCommand"),
        "error message must name the offending method, got: {}", err.message);
}
...
fn test_empty_method_error_code() {
    ...
    let resp = dispatch("", None);
    let err = resp.error.expect("empty method must yield error, not success");
    assert_eq!(err.code, -32601);
}
...
fn test_target_domain_create_target_returns_target_id() {
    ...
    assert_eq!(result["targetId"], TID, "createTarget echoes routed target_id");
}
...
fn test_target_domain_attach_returns_session_id() {
    ...
    let sid = result["sessionId"].as_str().expect("sessionId must be a string");
    assert!(!sid.is_empty(), "sessionId must be non-empty hex");
    // sessionId is hex-formatted; verify hex-only characters.
    assert!(sid.chars().all(|c| c.is_ascii_hexdigit()), "sessionId not hex: {}", sid);
}
...
fn test_target_domain_get_targets_returns_array() {
    ...
    let arr = result["targetInfos"].as_array().expect("targetInfos must be array");
    assert!(!arr.is_empty(), "getTargets returns at least the live target");
    let info = &arr[0];
...
fn test_page_domain_navigate_missing_url_defaults_to_blank() {
    ...
    assert!(resp.result.is_some(), "navigate without url must not fail (no bridge)");
}

#[test]
fn test_page_domain_capture_screenshot_without_bridge_returns_empty_data() {
    let resp = dispatch("Page.captureScreenshot", Some(json!({"format":"jpeg","quality":50})));
    let result = resp.result.expect("captureScreenshot without bridge returns stub");
...
fn assert_clone_debug<T: std::fmt::Debug + Clone>(label: &str, v: &T) {
    let cloned = v.clone();
    let dbg = format!("{:?}", v);
    assert!(dbg.contains(label), "Debug for {} missing '{}': {}", label, label, dbg);
...
fn test_bridge_cmd_create_target_clone_debug() {
    assert_clone_debug("CreateTarget",
...
fn test_bridge_cmd_debugger_list_frames_clone_debug() {
    assert_clone_debug("DebuggerListFrames",
        &BridgeCommand::DebuggerListFrames { target_id: TID.into() });
}

#[test]
fn test_bridge_cmd_debugger_get_environment_clone_debug() {
    assert_clone_debug("DebuggerGetEnvironment",
        &BridgeCommand::DebuggerGetEnvironment { target_id: TID.into(), frame_actor_id: "f1".into() });
}

#[test]
fn test_bridge_cmd_debugger_eval_clone_debug() {
    assert_clone_debug("DebuggerEval",
        &BridgeCommand::DebuggerEval { target_id: TID.into(), expression: "x".into(), frame_actor_id: None });
}
...
fn test_bridge_cmd_debugger_get_script_source_clone_debug() {
    assert_clone_debug("DebuggerGetScriptSource",
        &BridgeCommand::DebuggerGetScriptSource { target_id: TID.into(), script_id: 7 });
}

#[test]
fn test_bridge_cmd_debugger_blackbox_clone_debug() {
    assert_clone_debug("DebuggerBlackbox",
        &BridgeCommand::DebuggerBlackbox { target_id: TID.into(), script_id: 9 });
}

#[test]
fn test_bridge_cmd_debugger_unblackbox_clone_debug() {
    assert_clone_debug("DebuggerUnblackbox",
        &BridgeCommand::DebuggerUnblackbox { target_id: TID.into(), script_id: 9 });
}
...
fn test_bridge_cmd_navigate_empty_url_debug() {
    let cmd = BridgeCommand::Navigate { target_id: "".into(), url: "".into() };
    assert!(format!("{:?}", cmd).contains("Navigate"));
}
...
fn test_bridge_cmd_evaluate_empty_expression_debug() {
    let cmd = BridgeCommand::EvaluateJs {
        target_id: TID.into(), expression: "".into(), return_by_value: false,
    };
    let dbg = format!("{:?}", cmd);
    assert!(dbg.contains("EvaluateJs"));
    assert!(dbg.contains("false"), "return_by_value=false must appear in Debug");
}

#[test]
fn test_bridge_cmd_take_screenshot_quality_none_debug() {
    let cmd = BridgeCommand::TakeScreenshot {
        target_id: TID.into(), format: "jpeg".into(), quality: None,
    };
    assert!(format!("{:?}", cmd).contains("TakeScreenshot"));
}
...
fn test_bridge_cmd_take_screenshot_quality_max_u8() {
    ...
    for q in [0u8, 255u8] {
        let cmd = BridgeCommand::TakeScreenshot {
            target_id: TID.into(), format: "png".into(), quality: Some(q),
        };
        let dbg = format!("{:?}", cmd);
        assert!(dbg.contains(&q.to_string()), "quality={} must appear in Debug", q);
    }
}
...
fn test_bridge_cmd_set_viewport_zero_dimensions() {
    let cmd = BridgeCommand::SetViewport {
        target_id: TID.into(), width: 0, height: 0, device_scale_factor: Some(0.0),
    };
    let dbg = format!("{:?}", cmd);
...
fn test_bridge_cmd_set_viewport_no_dsf() {
    let cmd = BridgeCommand::SetViewport {
        target_id: TID.into(), width: 800, height: 600, device_scale_factor: None,
    };
    assert!(format!("{:?}", cmd).contains("SetViewport"));
}
...
fn test_bridge_cmd_dispatch_mouse_no_button_no_click_count() {
    let cmd = BridgeCommand::DispatchMouseEvent {
        target_id: TID.into(), event_type: "mouseMoved".into(),
        x: -1.0, y: -1.0, button: None, click_count: None,
    };
    let dbg = format!("{:?}", cmd);
...
fn test_bridge_cmd_dispatch_key_no_text() {
    let cmd = BridgeCommand::DispatchKeyEvent {
        target_id: TID.into(), event_type: "keyUp".into(),
        key: "Shift".into(), code: "ShiftLeft".into(), text: None,
    };
    assert!(format!("{:?}", cmd).contains("DispatchKeyEvent"));
}
...
fn test_bridge_cmd_set_cookie_minimal() {
    ...
    let cmd = BridgeCommand::SetCookie {
        target_id: TID.into(), name: "".into(), value: "".into(),
...
fn test_bridge_cmd_get_cookies_empty_urls_vec() {
    let cmd = BridgeCommand::GetCookies { target_id: TID.into(), urls: vec![] };
    let dbg = format!("{:?}", cmd);
...
fn test_bridge_cmd_debugger_resume_no_step_type() {
    let cmd = BridgeCommand::DebuggerResume { target_id: TID.into(), step_type: None };
...
fn test_bridge_cmd_debugger_eval_with_frame_actor() {
    let cmd = BridgeCommand::DebuggerEval {
        target_id: TID.into(), expression: "var x".into(),
...
fn test_bridge_response_err_contains_unicode() {
    ...
    let resp = BridgeResponse { result: Err(msg.clone()) };
    assert_eq!(resp.result.unwrap_err(), msg);
}

#[test]
fn test_bridge_response_debug_err_variant() {
    let resp = BridgeResponse { result: Err("boom".into()) };
    let dbg = format!("{:?}", resp);
    assert!(dbg.contains("boom"), "Debug of Err must contain message: {}", dbg);
}

#[test]
fn test_bridge_response_ok_complex_value() {
    let val = json!({"nested":{"arr":[1,2,3]},"n":42.5});
    let resp = BridgeResponse { result: Ok(val.clone()) };
...
fn test_channel_zero_duration_send_is_timeout_or_closed() {
    ...
    assert!(resp.result.is_err(),
        "send with timeout=0 and no responder must be an error, got: {:?}", resp.result);
}
...
fn test_channel_zero_duration_fire_and_forget_succeeds() {
    ...
    sender.send_fire_and_forget(BridgeCommand::GetTitle { target_id: TID.into() });
    let n = receiver.drain(|_| BridgeResponse { result: Ok(json!({})) });
    assert_eq!(n, 1, "fire-and-forget at timeout=0 must still deliver the command");
}
...
fn test_channel_clone_preserves_timeout_value() {
    ...
    let r1 = sender.send(BridgeCommand::GetTitle { target_id: TID.into() });
    let r2 = cloned.send(BridgeCommand::GetUrl { target_id: TID.into() });
    assert!(r1.result.is_err(), "original sender must time out");
    assert!(r2.result.is_err(), "cloned sender must enforce same timeout");
}
...
fn test_channel_multiple_clones_single_receiver() {
    ...
    s1.send_fire_and_forget(BridgeCommand::GetTitle { target_id: TID.into() });
    s2.send_fire_and_forget(BridgeCommand::GetUrl { target_id: TID.into() });
    sender.send_fire_and_forget(BridgeCommand::GetDocument { target_id: TID.into() });

    let count = receiver.drain(|_| BridgeResponse { result: Ok(json!({})) });
    assert_eq!(count, 3, "all three senders (orig + 2 clones) deliver to one receiver");
}
    ...
    let h = std::thread::spawn(move || {
        receiver.recv_and_process(Duration::from_secs(2), |cmd| match cmd {
            BridgeCommand::Navigate { url, .. } =>
                BridgeResponse { result: Ok(json!({"url": url})) },
            _ => BridgeResponse { result: Err("wrong cmd".into()) },
        })
    });

    let resp = sender.send(BridgeCommand::Navigate {
        target_id: TID.into(), url: "http://recv-proc".into(),
    });
...
fn test_channel_recv_and_process_timeout_false() {
    ...
    let processed = receiver.recv_and_process(Duration::from_millis(50),
        |_| BridgeResponse { result: Ok(json!({})) });
    let elapsed = start.elapsed();
    assert!(!processed, "recv_and_process with no sender must return false");
    assert!(elapsed >= Duration::from_millis(40),
        "recv_and_process must actually block until timeout (elapsed={:?})", elapsed);
}
...
fn test_channel_drain_returns_zero_after_consumer_already_emptied() {
    let (sender, receiver) = bridge_channel(Duration::from_secs(5));
    sender.send_fire_and_forget(BridgeCommand::GetTitle { target_id: TID.into() });
    let first = receiver.drain(|_| BridgeResponse { result: Ok(json!({})) });
    assert_eq!(first, 1);
    let second = receiver.drain(|_| BridgeResponse { result: Ok(json!({})) });
...
fn test_runtime_evaluate_with_expression_no_bridge_returns_stub_not_error() {
    ...
    let resp = dispatch("Runtime.evaluate", Some(json!({"expression":"1+1"})));
    assert!(resp.result.is_some(),
        "Runtime.evaluate without bridge returns stub, not error: {:?}", resp.error);
}

#[test]
fn test_emulation_set_ua_empty_no_bridge_no_send() {
    // Empty userAgent + no bridge → ok_empty (no send attempted).
    let resp = dispatch("Emulation.setUserAgentOverride", Some(json!({"userAgent":""})));
    assert!(resp.result.is_some(), "empty UA without bridge returns ok_empty");
}

#[test]
fn test_input_insert_text_empty_no_bridge_ok() {
    let resp = dispatch("Input.insertText", Some(json!({"text":""})));
    assert!(resp.result.is_some(), "empty text without bridge returns ok_empty");
}

#[test]
fn test_page_add_script_empty_source_no_bridge_ok() {
    // Empty source → bridge_send skipped (source.is_empty()), returns {"identifier":"1"}.
    let resp = dispatch("Page.addScriptToEvaluateOnNewDocument", Some(json!({"source":""})));
...
fn test_handle_command_uses_external_params_not_msg_params() {
    ...
    assert!(resp.result.is_some(), "dispatch must succeed using the params arg, not msg.params");
    assert_eq!(resp.result.unwrap()["frameId"], "0");
}
...
fn test_channel_high_volume_drain_preserves_count() {
    ...
    let count = receiver.drain(|_| BridgeResponse { result: Ok(json!({})) });
    assert_eq!(count, N, "mpsc channel must deliver all {} commands in order", N);
}
...
fn test_channel_send_then_drain_response_actually_delivered() {
    ...
    let h = std::thread::spawn(move || {
        receiver.drain(|cmd| match cmd {
            BridgeCommand::GetUrl { .. } =>
                BridgeResponse { result: Ok(json!("http://drained-url")) },
            _ => BridgeResponse { result: Err("unexpected".into()) },
        })
    });
    ...
    let _ = h.join();
    assert!(resp.result.is_ok(), "drain must route response back to sender via responder");
