    ...
    match cmd {
        BridgeCommand::EvaluateJs { ref expression, .. } => {
            if expression.contains("getBoundingClientRect") {
                ...
                BridgeResponse { result: Ok(json!(r#"{"matchedCSSRules":[],"inlineStyle":null}"#)) }
        ...
        BridgeCommand::QuerySelectorAll { .. } => BridgeResponse { result: Ok(json!({"nodeIds": [10, 11]})) },
        ...
        BridgeCommand::DebuggerSetBreakpoint { line, .. } => BridgeResponse {
            result: Ok(json!({"breakpointId": "bp-1", "actualLocation": {"scriptId": "1", "lineNumber": line, "columnNumber": 0}})),
        },
        BridgeCommand::DebuggerGetScriptSource { .. } => BridgeResponse { result: Ok(json!("")) },
...
fn setup_registry() -> DomainRegistry<DomainDispatch> {
    ...
    // Background responder: owns receiver exclusively, no race conditions
    std::thread::spawn(move || {
        loop {
            let handled = rx.try_process(|cmd| mock_bridge_response(cmd));
            if !handled {
...
#[test]
fn test_page_add_script_to_evaluate_with_bridge() {
    let reg = setup_registry();
