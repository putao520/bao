...
fn test_bridge_channel_short_timeout() {
    ...
    assert!(
        ...
        "short timeout must report 'timeout', got: {}",
        err
    );
}
...
fn test_bridge_channel_long_timeout() {
    ...
    assert!(
        resp.result.is_ok(),
        "long timeout must not fire for fast response"
    );
...
fn test_send_and_process_navigate() {
    ...
    let response = tx.send(BridgeCommand::Navigate {
        target_id: TID.into(),
        url: "http://test".into(),
    });
...
fn test_sender_clone_preserves_timeout_semantics() {
    ...
    let r1 = tx.send(BridgeCommand::GetTitle {
        target_id: TID.into(),
    });
    let r2 = tx2.send(BridgeCommand::GetUrl {
        target_id: TID.into(),
    });
    let elapsed = start.elapsed();
    assert!(
        r1.result.is_err() && r1.result.unwrap_err().contains("timeout"),
        "original sender must timeout"
    );
    assert!(
        r2.result.is_err() && r2.result.unwrap_err().contains("timeout"),
    ...
    assert!(
        ...
        "clone timeout inheritance broke: elapsed {:?}",
        elapsed
    );
}
...
fn test_sender_clone_long_timeout_does_not_fire() {
    ...
    let resp = tx2.send(BridgeCommand::GetTitle {
        target_id: TID.into(),
    });
    assert!(
        resp.result.is_ok(),
        "cloned long-timeout sender must not spuriously fail"
    );
    handler.join().unwrap();
}
...
fn test_drain_after_close_reports_zero() {
    ...
    let first = rx.drain(|_cmd| BridgeResponse {
        result: Ok(json!({})),
    });
    assert_eq!(first, 1);
    // Second drain on now-empty + sender-dropped channel must be 0 cleanly.
    let second = rx.drain(|_cmd| BridgeResponse {
        result: Ok(json!({})),
    });
    assert_eq!(second, 0);
}
...
fn test_bridge_command_create_target_debug() {
    let cmd = BridgeCommand::CreateTarget {
        url: "http://new.target".into(),
    };
    let debug = format!("{:?}", cmd);
...
fn test_bridge_command_debugger_enable_debug() {
    let cmd = BridgeCommand::DebuggerEnable {
        target_id: TID.into(),
    };
...
fn test_bridge_command_debugger_disable_debug() {
    let cmd = BridgeCommand::DebuggerDisable {
        target_id: TID.into(),
    };
...
fn test_bridge_command_debugger_remove_breakpoint_debug() {
    let cmd = BridgeCommand::DebuggerRemoveBreakpoint {
        target_id: TID.into(),
...
fn test_bridge_command_debugger_interrupt_debug() {
    let cmd = BridgeCommand::DebuggerInterrupt {
        target_id: TID.into(),
    };
...
fn test_bridge_command_debugger_list_frames_debug() {
    let cmd = BridgeCommand::DebuggerListFrames {
        target_id: TID.into(),
    };
...
fn test_bridge_command_debugger_get_environment_debug() {
    let cmd = BridgeCommand::DebuggerGetEnvironment {
        target_id: TID.into(),
        frame_actor_id: "actor-1".into(),
    };
    let debug = format!("{:?}", cmd);
...
fn test_bridge_command_debugger_eval_debug() {
    let cmd = BridgeCommand::DebuggerEval {
        target_id: TID.into(),
        expression: "debugger".into(),
        frame_actor_id: Some("actor-2".into()),
    };
    let debug = format!("{:?}", cmd);
...
fn test_bridge_command_debugger_get_possible_breakpoints_debug() {
    let cmd = BridgeCommand::DebuggerGetPossibleBreakpoints {
        target_id: TID.into(),
        start_script_id: "11".into(),
    };
...
fn test_bridge_command_debugger_get_script_source_debug() {
    let cmd = BridgeCommand::DebuggerGetScriptSource {
        target_id: TID.into(),
        script_id: 13,
    };
...
fn test_bridge_command_debugger_blackbox_debug() {
    let cmd = BridgeCommand::DebuggerBlackbox {
        target_id: TID.into(),
        script_id: 17,
    };
...
fn test_bridge_command_debugger_unblackbox_debug() {
    let cmd = BridgeCommand::DebuggerUnblackbox {
        target_id: TID.into(),
        script_id: 19,
    };
...
fn test_all_bridge_commands_clone_round_trip() {
    ...
    let originals: Vec<BridgeCommand> = vec![
        ...
        BridgeCommand::DebuggerGetEnvironment {
            target_id: TID.into(),
            frame_actor_id: "fa".into(),
        },
        BridgeCommand::DebuggerEval {
            target_id: TID.into(),
            expression: "e".into(),
            frame_actor_id: Some("fa".into()),
        },
        BridgeCommand::DebuggerGetPossibleBreakpoints {
            target_id: TID.into(),
    ...
    let drained = rx.drain(|_cmd| BridgeResponse {
        result: Ok(json!({})),
    });
    assert_eq!(
        drained, 41,
        "all 41 cloned variants must traverse the channel"
    );
}
...
fn test_sequential_send_recv() {
    ...
    let r1 = tx.send(BridgeCommand::Navigate {
        target_id: TID.into(),
        url: "http://a.com".into(),
    });
    ...
    let r3 = tx.send(BridgeCommand::Navigate {
        target_id: TID.into(),
        url: "http://b.com".into(),
    });
...
fn test_recv_and_process_blocks_until_command() {
    ...
    let processed = rx.recv_and_process(Duration::from_secs(2), |_cmd| BridgeResponse {
        result: Ok(json!({"blocked": true})),
    });
...
fn test_recv_and_process_returns_false_on_timeout() {
    ...
    let (_tx, rx) = bridge_channel(Duration::from_secs(5));
    let start = std::time::Instant::now();
    let processed = rx.recv_and_process(Duration::from_millis(50), |_cmd| BridgeResponse {
        result: Ok(json!({})),
    });
    let elapsed = start.elapsed();
    ...
    assert!(
        ...
        "must actually wait for the timeout, elapsed {:?}",
        elapsed
    );
    assert!(
...
fn test_handler_error_propagates_through_channel() {
    ...
    let handler = std::thread::spawn(move || loop {
        let processed = rx.try_process(|_cmd| BridgeResponse {
            result: Err("handler-internal-failure-42".into()),
        });
        if processed {
            break;
        }
...
fn test_send_after_receiver_dropped_with_clone() {
    ...
    assert!(
        r1.result.is_err() && r1.result.unwrap_err().contains("closed"),
        "original sender must see closed channel"
    );
    assert!(
        r2.result.is_err() && r2.result.unwrap_err().contains("closed"),
...
fn test_concurrent_cloned_senders_interleave() {
    ...
    assert_eq!(
        drained,
        2 * N,
        "concurrent cloned senders must deliver all {} commands, got {}",
        2 * N,
        drained
    );
}
...
fn test_send_returns_closed_not_timeout_when_receiver_dropped() {
    ...
    assert!(
        err.contains("closed"),
        "dropped receiver must report 'closed', got: {}",
        err
    );
    assert!(
        !err.contains("timeout"),
        "closed channel must NOT masquerade as timeout"
    );
}
...
fn test_fire_and_forget_is_non_blocking_under_load() {
    ...
    assert!(
        elapsed < Duration::from_secs(2),
        "fire-and-forget must be non-blocking under load, elapsed {:?}",
        elapsed
    );
}
...
fn test_response_responder_isolated_per_request() {
    ...
    let r2 = tx.send(BridgeCommand::GetUrl {
        target_id: TID.into(),
    });
    assert_eq!(
        r1.result.unwrap(),
        1,
        "first response must be the first handler result"
    );
    assert_eq!(
        r2.result.unwrap(),
        2,
        "second response must be the second handler result"
    );
    handler.join().unwrap();
}
