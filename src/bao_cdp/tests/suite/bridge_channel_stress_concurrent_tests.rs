...
fn test_burst_100_commands_drain_all() {
    ...
    assert_eq!(
        ...
        100,
        "handler-side counter must equal 100"
    );
    assert_eq!(
        drained, 100,
        "drain() return value must equal handler invocation count"
    );
    // G7: FIFO order — last drained URL should be index 99
    assert_eq!(
        *last_url.lock().unwrap(),
        "http://example.com/99",
...
fn test_burst_mixed_command_types() {
    ...
    for i in 0..30 {
        match i % 3 {
            0 => tx.send_fire_and_forget(BridgeCommand::Navigate {
                target_id: TID.into(),
                url: format!("http://x/{}", i),
            }),
            1 => tx.send_fire_and_forget(BridgeCommand::EvaluateJs {
                target_id: TID.into(),
                expression: format!("{}", i),
                return_by_value: true,
            }),
    ...
    let drained = rx.drain(|cmd| {
        match cmd {
            BridgeCommand::Navigate { .. } => {
                nav.fetch_add(1, Ordering::SeqCst);
            }
    ...
    assert_eq!(
        nav.load(Ordering::SeqCst),
        10,
        "Navigate count (i%3==0 → 0,3,...,27 = 10)"
    );
    ...
    let total =
        nav.load(Ordering::SeqCst) + eval.load(Ordering::SeqCst) + title.load(Ordering::SeqCst);
    assert_eq!(
        total, 30,
        "sum of per-variant counters must equal total sent"
    );
    assert_eq!(
        drained as u32, total,
        "drain() return must equal sum of per-variant counters"
    );
}
...
fn test_concurrent_sends_from_4_threads() {
    ...
    assert_eq!(
        count.load(Ordering::SeqCst),
        100,
        "total drained must equal 4*25"
    );
    let counts = per_thread.lock().unwrap();
    for t in 0..4 {
        assert_eq!(
            counts[t], 25,
            "thread {} must have exactly 25 commands drained",
            t
        );
    }
}
...
fn test_concurrent_send_with_sync_response() {
        ...
        senders.push(std::thread::spawn(move || {
            for i in 0..10 {
                if i % 2 == 0 {
                    ...
                    assert!(
                        resp.result.is_ok(),
                        "thread {} iter {} sync send must succeed, got {:?}",
                        t,
                        i,
                        resp.result
                    );
    ...
    assert_eq!(
        sync_ok, 20,
        "exactly 20 sync GetTitle sends (4 threads * 5 even iterations) must succeed"
    );
    // processed >= sync_ok because responder also processes fire-and-forget Navigate commands
    assert!(
        processed >= sync_ok,
        "processed ({}) must be >= sync successes ({}) — responder also handles fire-and-forget",
        processed,
        sync_ok
    );
}
...
fn test_timeout_when_no_responder() {
    ...
    assert!(
        ...
        "send must block at least ~timeout (45ms) before returning, took {:?}",
        elapsed
    );
    assert!(
        elapsed <= Duration::from_millis(500),
        "send must not vastly exceed timeout bound, took {:?}",
        elapsed
    );
}
...
fn test_timeout_message_format() {
    let (tx, _rx) = setup(20);
    let resp = tx.send(BridgeCommand::Navigate {
        target_id: TID.into(),
        url: "http://x".into(),
    });
    let err = resp.result.unwrap_err();
    assert!(
        err.contains("timeout") || err.contains("bridge"),
        "error must mention 'timeout' or 'bridge', got: {}",
        err
    );
}
...
fn test_fire_and_forget_does_not_block() {
    ...
    assert!(
        ...
        "fire-and-forget should be fast, took {:?}",
        elapsed
    );
}
...
fn test_cloned_sender_sends_to_same_receiver() {
    ...
    tx2.send_fire_and_forget(BridgeCommand::Navigate {
        target_id: TID.into(),
        url: "http://x".into(),
    });
...
fn test_multiple_cloned_senders_concurrent() {
    ...
    assert_eq!(
        drained, 4,
        "drain return must equal number of cloned senders"
    );
}
...
fn test_all_bridge_command_variants_serializable() {
    let commands: Vec<BridgeCommand> = vec![
        ...
        BridgeCommand::DebuggerGetEnvironment {
            target_id: TID.into(),
            frame_actor_id: "frame1".into(),
        },
        BridgeCommand::DebuggerEval {
            target_id: TID.into(),
            expression: "x".into(),
            frame_actor_id: Some("frame1".into()),
        },
        BridgeCommand::DebuggerGetPossibleBreakpoints {
            target_id: TID.into(),
            start_script_id: "1".into(),
        },
    ...
    assert_eq!(
        drained_count, 41,
        "expected all 41 BridgeCommand variants drained, got {}",
        drained_count
    );
    assert_eq!(drained, total, "drain() return must equal commands sent");
    assert_eq!(
        total, 41,
        "test vector must enumerate all 41 enum variants (full coverage)"
    );
}
        ...
        for _ in 0..100 {
            let processed = {
                ...
                rx_guard.try_process(|cmd| match cmd {
                    BridgeCommand::GetTitle { .. } => BridgeResponse {
                        result: Ok(json!("My Title")),
                    },
                    BridgeCommand::GetUrl { .. } => BridgeResponse {
                        result: Ok(json!("http://example.com")),
                    },
                    _ => BridgeResponse {
                        result: Ok(json!({})),
                    },
        ...
        for _ in 0..100 {
            let processed = {
                ...
                rx_guard.try_process(|_| BridgeResponse {
                    result: Err("internal error".into()),
                })
            };
            if processed {
                done2.store(true, Ordering::SeqCst);
...
fn test_try_process_empty_returns_false() {
    let (_tx, rx) = setup(500);
    assert!(!rx.try_process(|_| BridgeResponse {
        result: Ok(json!({}))
    }));
}
...
fn test_drain_preserves_fifo_order() {
    ...
    for (i, t) in order.iter().enumerate() {
        assert_eq!(
            t,
            &format!("seq-{:03}", i),
            "FIFO violation at index {}: expected seq-{:03}, got {}",
            i,
            i,
            t
        );
...
fn test_zero_timeout_returns_err_immediately() {
    ...
    assert!(
        resp.result.is_err(),
        "zero timeout must yield error (no responder ready in 0ms)"
    );
    // G4: zero timeout must return near-instantly (allow scheduling slack)
    assert!(
        elapsed <= Duration::from_millis(50),
        "zero-timeout send must not block long, took {:?}",
        elapsed
    );
}
...
fn test_send_after_receiver_dropped_returns_channel_closed() {
    ...
    assert!(
        err.contains("closed") || err.contains("timeout"),
        "send after receiver dropped must error with closed/timeout, got: {}",
        err
    );
}
...
fn test_get_cookies_empty_urls_boundary() {
    let (tx, rx) = setup(500);
    tx.send_fire_and_forget(BridgeCommand::GetCookies {
        target_id: TID.into(),
        urls: vec![],
    });
    ...
    assert!(
        got_empty.load(Ordering::SeqCst),
        "must have processed the GetCookies boundary command"
    );
}
...
fn test_take_screenshot_quality_boundary_none() {
    ...
    assert!(
        ...
        "must process TakeScreenshot with quality=None"
    );
}
...
fn test_take_screenshot_quality_boundary_extremes() {
    ...
    assert_eq!(
        qualities.len(),
        2,
        "both quality boundaries must be drained"
    );
    assert!(
        qualities.contains(&Some(0)),
        "quality=0 boundary must survive"
    );
    assert!(
        qualities.contains(&Some(100)),
        "quality=100 boundary must survive"
    );
}
...
fn test_set_viewport_zero_dimensions_boundary() {
    ...
    rx.drain(|cmd| {
        ...
        } = cmd
        {
            ...
            assert!(
                ...
                "device_scale_factor=None boundary must roundtrip"
            );
    ...
    assert!(
        got_zero.load(Ordering::SeqCst),
        "must process SetViewport with zero dimensions"
    );
}
...
fn test_set_viewport_f64_precision_preserved() {
    ...
    assert!(
        got_precise.load(Ordering::SeqCst),
        "must process SetViewport with fractional dsf"
    );
}
...
fn test_dispatch_mouse_event_extreme_coordinates() {
    ...
    assert_eq!(
        seen.len(),
        3,
        "all 3 f64 boundary coordinates must be drained"
    );
    for (x, y) in seen.iter() {
        assert!(
            coords.contains(x) && coords.contains(y),
            "f64 coordinate must roundtrip exactly: {:?}",
            (x, y)
        );
    }
}
...
fn test_large_payload_stress() {
    ...
    rx.drain(move |cmd| {
        if let BridgeCommand::EvaluateJs { expression, .. } = cmd {
            assert_eq!(
                expression.len(),
                64 * 1024,
                "payload length must survive transit"
            );
            assert_eq!(
                expression, expected,
                "payload bytes must be byte-identical after transit"
            );
    ...
    assert!(
        got_exact.load(Ordering::SeqCst),
        "must process the large-payload command"
    );
}
...
fn test_drain_send_drain_sequence() {
    ...
    for i in 0..2 {
        tx.send_fire_and_forget(BridgeCommand::GetTitle {
            target_id: format!("p3-{}", i),
        });
    }
    let last = rx.drain(|_| BridgeResponse {
        result: Ok(json!({})),
    });
    assert_eq!(last, 2, "final drain must get exactly 2");

    // Phase 4: drain again — must be 0
    let after = rx.drain(|_| BridgeResponse {
        result: Ok(json!({})),
    });
...
fn test_is_alive_after_all_clones_dropped_but_receiver_alive() {
    ...
    assert!(
        tx.is_alive(),
        "original sender must stay alive while receiver exists"
    );
    drop(rx);
    assert!(
        !tx.is_alive(),
        "after receiver dropped, sender must report dead"
    );
}
...
fn test_try_process_single_command_exact_roundtrip() {
    ...
    tx.send_fire_and_forget(BridgeCommand::Navigate {
        target_id: TID.into(),
        url: "https://example.org/path?q=1".into(),
    });
...
fn test_burst_send_then_single_drain_no_loss() {
    ...
    assert_eq!(
        count, 200,
        "single drain after 200 sends must recover all 200 — mpsc guarantees no loss"
    );
    // Follow-up drain must be empty
    let recheck = rx.drain(|_| BridgeResponse {
        result: Ok(json!({})),
    });
