...
fn test_bridge_take_screenshot_png() {
    let cmd = BridgeCommand::TakeScreenshot { target_id: TID.into(), format: "png".into(), quality: None };
    let _ = format!("{:?}", cmd);
}

#[test]
fn test_bridge_take_screenshot_jpeg_with_quality() {
    let cmd = BridgeCommand::TakeScreenshot { target_id: TID.into(), format: "jpeg".into(), quality: Some(80) };
    let _ = format!("{:?}", cmd);
}

#[test]
fn test_bridge_get_title() {
    let _ = format!("{:?}", BridgeCommand::GetTitle { target_id: TID.into() });
}

#[test]
fn test_bridge_get_url() {
    let _ = format!("{:?}", BridgeCommand::GetUrl { target_id: TID.into() });
}

#[test]
fn test_bridge_get_document() {
    let _ = format!("{:?}", BridgeCommand::GetDocument { target_id: TID.into() });
}
...
fn test_bridge_query_selector_all() {
    let cmd = BridgeCommand::QuerySelectorAll { target_id: TID.into(), selector: "a[href]".into() };
    let _ = format!("{:?}", cmd);
}
...
fn test_bridge_set_attribute_value() {
    let cmd = BridgeCommand::SetAttributeValue { target_id: TID.into(), node_id: 1,
        name: "class".into(),
        value: "active".into(), };
    let _ = format!("{:?}", cmd);
}
...
fn test_bridge_dispatch_mouse_move() {
    let cmd = BridgeCommand::DispatchMouseEvent { target_id: TID.into(), event_type: "mouseMoved".into(),
        x: 150.0,
        y: 250.0,
        button: None,
        click_count: None, };
...
fn test_bridge_insert_text() {
    let cmd = BridgeCommand::InsertText { target_id: TID.into(), text: "hello world".into() };
    let _ = format!("{:?}", cmd);
}

#[test]
fn test_bridge_set_viewport() {
    let cmd = BridgeCommand::SetViewport { target_id: TID.into(), width: 1920,
        height: 1080,
...
fn test_bridge_set_viewport_no_dsf() {
    let cmd = BridgeCommand::SetViewport { target_id: TID.into(), width: 800,
        height: 600,
...
fn test_bridge_set_user_agent() {
    let cmd = BridgeCommand::SetUserAgent { target_id: TID.into(), user_agent: "Mozilla/5.0".into() };
    let _ = format!("{:?}", cmd);
}

#[test]
fn test_bridge_get_cookies() {
    let cmd = BridgeCommand::GetCookies { target_id: TID.into(), urls: vec!["https://a.com".into(), "https://b.com".into()] };
    let _ = format!("{:?}", cmd);
}

#[test]
fn test_bridge_get_all_cookies() {
    let _ = format!("{:?}", BridgeCommand::GetAllCookies { target_id: TID.into() });
}
...
fn test_bridge_set_cookie() {
    let cmd = BridgeCommand::SetCookie { target_id: TID.into(), name: "token".into(),
        value: "abc123".into(),
...
fn test_bridge_go_back() {
    let _ = format!("{:?}", BridgeCommand::GoBack { target_id: TID.into() });
}

#[test]
fn test_bridge_go_forward() {
    let _ = format!("{:?}", BridgeCommand::GoForward { target_id: TID.into() });
}

#[test]
fn test_bridge_stop_loading() {
    let _ = format!("{:?}", BridgeCommand::StopLoading { target_id: TID.into() });
}

#[test]
fn test_bridge_close_page() {
    let _ = format!("{:?}", BridgeCommand::ClosePage { target_id: TID.into() });
}
...
fn test_bridge_debug_navigate_contains_url() {
    let debug = format!("{:?}", BridgeCommand::Navigate { target_id: TID.into(), url: "https://test.com".into() });
    assert!(debug.contains("test.com"));
}

#[test]
fn test_bridge_debug_evaluate_contains_expression() {
    let debug = format!("{:?}", BridgeCommand::EvaluateJs { target_id: TID.into(), expression: "42".into(), return_by_value: false });
...
fn test_bridge_navigate_empty_url() {
    let cmd = BridgeCommand::Navigate { target_id: TID.into(), url: String::new() };
    let _ = format!("{:?}", cmd);
}

#[test]
fn test_bridge_evaluate_js_empty() {
    let cmd = BridgeCommand::EvaluateJs { target_id: TID.into(), expression: String::new(), return_by_value: false };
    let _ = format!("{:?}", cmd);
}

#[test]
fn test_bridge_insert_text_empty() {
    let cmd = BridgeCommand::InsertText { target_id: TID.into(), text: String::new() };
    let _ = format!("{:?}", cmd);
}

#[test]
fn test_bridge_insert_text_unicode() {
    let cmd = BridgeCommand::InsertText { target_id: TID.into(), text: "日本語テスト".into() };
...
fn test_bridge_query_selector_complex() {
    let cmd = BridgeCommand::QuerySelector { target_id: TID.into(), selector: "div.class > ul li:nth-child(2) a[href^='https://']".into(), };
    let debug = format!("{:?}", cmd);
...
fn test_bridge_set_viewport_zero_dims() {
    let cmd = BridgeCommand::SetViewport { target_id: TID.into(), width: 0, height: 0, device_scale_factor: Some(0.0) };
    let _ = format!("{:?}", cmd);
}
...
fn test_bridge_fire_and_forget() {
    let (tx, rx) = bridge_channel(Duration::from_secs(5));
    tx.send_fire_and_forget(BridgeCommand::StopLoading { target_id: TID.into() });
    drop(rx);
