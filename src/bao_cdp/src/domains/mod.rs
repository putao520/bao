...
impl cdp_server::TargetProvider for ServoTargetProvider {
    fn list_targets(&self) -> Vec<TargetInfo> {
                ...
                return arr
                    ...
                    .filter_map(|entry| {
                        ...
                        let url = entry
                            ...
                            .to_string();
                        let ws_url =
                            format!("ws://{}:{}/devtools/page/{}", self.host, self.port, id);
                        Some(TargetInfo {
                            id,
        ...
        vec![TargetInfo {
            ...
            web_socket_debugger_url: format!(
                "ws://{}:{}/devtools/page/{}",
                self.host, self.port, self.target_id
            ),
        }]
    }
    ...
    fn create_target(&self, url: &str) -> Result<TargetInfo, String> {
        let result = self
            .bridge
            .send(BridgeCommand::CreateTarget {
                url: url.to_string(),
            })
            .result;
        match result {
            Ok(val) => {
                ...
                if new_id == self.target_id {
                    return Err(
                        "CreateTarget returned existing targetId, not a new page".to_string()
                    );
                }
                Ok(TargetInfo {
                    id: new_id.to_string(),
...
mod tests {
    ...
    fn list_targets_fallback_when_no_responder() {
        ...
        let (bridge, _rx) = bridge_channel(TIMEOUT);
        let provider =
            ServoTargetProvider::new(bridge, "default-target".into(), "127.0.0.1".into(), 9222);
        ...
        assert!(targets[0]
            ...
            .contains("/devtools/page/default-target"));
    }
    ...
    fn create_target_rejects_existing_target_id() {
        ...
        assert!(
            result.is_err(),
            "should reject fallback to existing target_id"
        );
    }
