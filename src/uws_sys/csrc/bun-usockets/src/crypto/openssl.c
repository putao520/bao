...
static int us_install_ca_buffers(SSL_CTX *ctx, X509_STORE *store, STACK_OF(CRYPTO_BUFFER) *certs) {
  ...
  /* BAO PORT: upstream stores the CA set through BoringSSL's lazy-cert API
   * (X509_LAZY_CERT_SET_* / X509_STORE_add_lazy_cert_set), which the vendored
   * BoringSSL (src/boringssl_sys — frozen) predates. Parse eagerly instead:
   * the trust set and the client-CA subject list are identical, only the
   * parse timing differs (upfront vs on first lookup). */
  STACK_OF(CRYPTO_BUFFER) *names = n > 0 ? sk_CRYPTO_BUFFER_new_null() : NULL;
  if (n > 0 && names == NULL) ok = 0;
  for (size_t i = 0; ok && i < n; i++) {
    CRYPTO_BUFFER *buf = sk_CRYPTO_BUFFER_value(certs, i);
    X509 *x509 = X509_parse_from_buffer(buf);
    ok = x509 != NULL && X509_STORE_add_cert(store, x509);
    if (ok) {
      int subject_len = i2d_X509_NAME(X509_get_subject_name(x509), NULL);
      unsigned char *subject = subject_len > 0 ? (unsigned char *)OPENSSL_malloc((size_t)subject_len) : NULL;
      unsigned char *subject_out = subject;
      ok = subject != NULL && i2d_X509_NAME(X509_get_subject_name(x509), &subject_out) == subject_len;
      if (ok) {
        CRYPTO_BUFFER *name = CRYPTO_BUFFER_new(subject, (size_t)subject_len, NULL);
        if (name != NULL) {
          sk_CRYPTO_BUFFER_push(names, name);
        } else {
          ok = 0;
        }
      }
...
SSL_CTX *us_ssl_ctx_build_raw(struct us_bun_socket_context_options_t options,
  ...
  /* BAO PORT: the vendored BoringSSL (src/boringssl_sys — frozen) predates
   * X509_V_FLAG_IGNORE_EXPIRED_TRUST_ANCHORS; define it to 0 so the flag set
   * is a no-op and expired-anchor handling keeps BoringSSL's default. */
#ifndef X509_V_FLAG_IGNORE_EXPIRED_TRUST_ANCHORS
#define X509_V_FLAG_IGNORE_EXPIRED_TRUST_ANCHORS 0
#endif
