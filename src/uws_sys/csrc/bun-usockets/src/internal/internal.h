...
/* BAO PORT: the symbol is extern "C" everywhere (defined in Rust,
 * src/uws_sys/src/c_hooks.rs; bun-uws Loop.h also declares it extern "C").
 * Upstream's decl here carries C++ linkage, which conflicts with Loop.h's
 * when a C++ TU includes both — pin it to the real linkage. */
#ifdef __cplusplus
