...
namespace uWS
{
    ...
    public:
         ...
         * whatever the length of the read (upstream bun 663508d6d6, #43153). */
    ...
    HttpParserResult fenceAndConsumePostPadded(uint64_t maxHeaderSize, bool& isConnectRequest, bool requireHostHeader, bool useStrictMethodValidation, bool isNodeHttp, char *data, unsigned int length, void *user, HttpRequest *req, MoveOnlyFunction<void *(void *, HttpRequest *)> &requestHandler, MoveOnlyFunction<void *(void *, std::string_view, bool)> &dataHandler) {
        ...
        for (;length;) {
            ...
            * ought to be handled as an error. */
             ...
             * consistent with the TE+CL and non-chunked TE rejections below, so a
             * proxy/backend split on whether HTTP/1.0 honours Transfer-Encoding cannot
             * desync on the body boundary. node:http follows llhttp, which dispatches
             * the request (the HTTP/1.0 request already marks the connection for close
             * via HttpContext's isAncient handling). Mirrors Bun upstream's !IsNodeHttp
             * template split via the runtime HttpFlags::isNodeHttp. */
