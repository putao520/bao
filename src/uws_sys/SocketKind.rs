...
/// Referenced from `openssl.c` (absorbed oven-sh/bun 4af1842c8c) so the
/// new-session callback's per-SSL marker is only set for the sockets that
/// actually surface the `'session'` event.
#[unsafe(no_mangle)]
