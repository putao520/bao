...
/// `open(2)` flag for "not visible to child processes". POSIX spells it
/// `O_CLOEXEC`; Windows has no exec-time close to defer, and the CRT's exact
/// predicate is `_O_NOINHERIT` (the handle is not inherited by children).
#[cfg(not(windows))]
const O_CLOEXEC: libc::c_int = libc::O_CLOEXEC;
#[cfg(windows)]
const O_CLOEXEC: libc::c_int = libc::O_NOINHERIT;
...
pub fn read_to_string(path: &str) -> Result<String, std::io::Error> {
    ...
    let file = File::open(zpath.as_zstr(), libc::O_RDONLY | O_CLOEXEC, 0)
...
pub fn write(path: &str, data: &[u8]) -> Result<(), std::io::Error> {
    ...
    let file = File::open(
        zpath.as_zstr(),
        libc::O_WRONLY | libc::O_CREAT | libc::O_TRUNC | O_CLOEXEC,
        0o666 as Mode,
    )
    .map_err(|e| std::io::Error::from_raw_os_error(e.errno as i32))?;
