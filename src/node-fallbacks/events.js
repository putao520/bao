...
function once(emitter, type, options) {
  ...
  // upstream 7b31eddfb8: `$newPromiseCapability` is a JSC builtin intrinsic that is
  // only rewritten inside src/js runtime builtins — in a browser polyfill bundle it
  // is a lookup of a global that does not exist. Build the promise with a standard
  // executor instead (same shape node uses).
