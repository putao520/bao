...
pub fn install(cx: &mut mozjs::context::JSContext) {
    ...
    unsafe {
        ...
        {
            let Some(_opts_guard) = crate::compile_options_guard::CompileOptionsGuard::new(mozjs::glue::NewCompileOptions(cx.raw_cx(), c"node:http".as_ptr(), 1) as *mut _) else { return; };
            let opts = _opts_guard.as_ptr();
                let mut src_text = mozjs::rust::transform_str_to_source_text(
                    "function Server(opts, cb) { if (typeof opts === 'function') { cb = opts; } if (cb) this.on('request', cb); }\
                     ...
                     (function(){var EE=null;try{EE=require('events').EventEmitter}catch(e){EE=function EE(){this._events={}};EE.prototype.on=function(e,fn){(this._events[e]||(this._events[e]=[])).push(fn);return this};EE.prototype.emit=function(e){var a=Array.prototype.slice.call(arguments,1);var ls=this._events[e];if(ls)for(var i=0;i<ls.length;i++)ls[i].apply(this,a);return!!ls}}Server.prototype=Object.create(EE.prototype);Server.prototype.constructor=Server})();\
                     Server"
                );
                let mut rval = UndefinedValue();
                JS::Evaluate2(cx.raw_cx(), opts as *const _, &mut src_text, MutableHandle::<Value> {
        ...
        if !status_obj.get().is_null() {
            ...
            for (code, msg) in codes {
                let c_code = bun_core::ZBox::from_bytes(code.as_bytes());
...

        {
            ...
            let c_methods = bun_core::ZBox::from_bytes(methods.as_bytes());
...
unsafe extern "C" fn uws_route_handler(
    ...
    let c_method = bun_core::ZBox::from_bytes(method_bytes);
    ...
    let c_url = bun_core::ZBox::from_bytes(url_bytes);
    ...
    if !headers_obj.get().is_null() {
        ...
        for &name in common_headers {
            if let Some(value) = req_ref.header(name) {
                let c_k = bun_core::ZBox::from_bytes(name);
                let c_v = bun_core::ZBox::from_bytes(value);
            ...
            if argc > 1 {
                ...
                if hdrs_val.is_object() {
                    ...
                    if !uws_res.is_null() {
                        ...
                        for &key in common {
                            let c_key = bun_core::ZBox::from_bytes(key);
                            ...
                            if hv.is_string() {
                                ...
                                let c_val = bun_core::ZBox::from_bytes(val.as_str().as_bytes());
...
unsafe extern "C" fn res_write(
    ...
    if argc > 0 {
        ...
        if v.is_string() {
            ...
            let c_combined = bun_core::ZBox::from_bytes(combined.as_bytes());
...
unsafe extern "C" fn server_listen(
    ...
    let app_ptr = match App::<false>::create(&opts) {
        Some(p) => p,
        None => {
            let c_msg = bun_core::ZBox::from_bytes(format!("Failed to create HTTP server on port {}", port).as_bytes());
    ...
    if global.is_null() || !handler_val.is_object() {
        ...
        let msg = c"http.createServer requires a request handler".as_ptr();
...
unsafe extern "C" fn server_address(
    ...
    if port_val.is_int32() {
        ...
        let c_family = c"IPv4".as_ptr();
        let js_family = JS_NewStringCopyZ(cx, c_family);
        ...
        let c_addr = c"0.0.0.0".as_ptr();
        let js_addr = JS_NewStringCopyZ(cx, c_addr);
        if !js_addr.is_null() {
...
unsafe extern "C" fn http_request(
    ...
    let c_url = bun_core::ZBox::from_bytes(url_str.as_bytes());
    ...
    let c_method = bun_core::ZBox::from_bytes(method.as_bytes());
