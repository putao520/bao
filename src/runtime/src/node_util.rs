...
t.isExternal=function(){return false};
                ...
                let Some(_opts_guard) = crate::compile_options_guard::CompileOptionsGuard::new(opts as *mut _) else {
                ...
                let opts = _opts_guard.as_ptr();
                    ...
                    if !global.is_null() && JS::Evaluate2(cx.raw_cx(), opts as *const _, &mut src, factory_h) && factory_val.is_object() {
...
pub fn install_assert(cx: &mut mozjs::context::JSContext) {
  ...
  return api;
        ...
        let Some(_opts_guard) = crate::compile_options_guard::CompileOptionsGuard::new(mozjs::glue::NewCompileOptions(cx.raw_cx(), c"assert".as_ptr(), 1) as *mut _) else {
            return;
        };
        let opts = _opts_guard.as_ptr();
        let ok = JS::Evaluate2(cx.raw_cx(), opts as *const _, &mut src_text, rval_h);
...
unsafe extern "C" fn util_promisify(cx: *mut JSContext, _argc: u32, vp: *mut JSVal) -> bool {
  ...
  return function promisified() {
    ...
    let mut factory_val = UndefinedValue();
    let factory_h = MutableHandle::<Value> { _phantom_0: ::std::marker::PhantomData, ptr: &mut factory_val };
    let Some(_opts_guard) = crate::compile_options_guard::CompileOptionsGuard::new(mozjs::glue::NewCompileOptions(cx, c"<promisify>".as_ptr(), 1) as *mut _) else {
        args.rval().set(*args.get(0).ptr);
        return true;
    };
    let opts = _opts_guard.as_ptr();
    if !JS::Evaluate2(cx, opts as *const _, &mut src, factory_h) || !factory_val.is_object() {
        args.rval().set(*args.get(0).ptr);
        return true;
    }
