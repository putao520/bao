...
/// Build `__gc_cache_{key}` as a ZBox (single allocation, no format! overhead).
/// D62: ZBox::from_bytes replaces CString::new(format!(...)) double-allocation.
fn gc_cache_prop_name(key: &str) -> bun_core::ZBox {
    let prefix = b"__gc_cache_";
    let key_bytes = key.as_bytes();
    let mut bytes = Vec::with_capacity(prefix.len() + key_bytes.len());
...
impl GcStore {
    ...
    fn insert(&mut self, cx: *mut JSContext, key: &str, obj: *mut JSObject) {
        ...
        let prop_name = gc_cache_prop_name(key);
