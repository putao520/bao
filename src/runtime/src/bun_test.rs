...
unsafe fn eval_shim(raw: *mut JSContext, source: &str, label: &str) {
    // D75: ZBox::from_bytes replaces CString::new(format!(...)) double-allocation.
    let c_filename = {
        let prefix = b"<";
        let suffix = b"-shim>";
        let mut bytes = Vec::with_capacity(prefix.len() + label.len() + suffix.len());
        bytes.extend_from_slice(prefix);
        bytes.extend_from_slice(label.as_bytes());
        bytes.extend_from_slice(suffix);
        bun_core::ZBox::from_bytes(bytes)
    };
    let Some(_opts_guard) = crate::compile_options_guard::CompileOptionsGuard::new(mozjs::glue::NewCompileOptions(raw, c_filename.as_ptr(), 1) as *mut _) else {
...
unsafe fn eval_shim_get_obj(raw: *mut JSContext, expr: &str) -> *mut JSObject {
    let c_filename = bun_core::ZBox::from_bytes(b"<shim-get>\0".as_slice().to_vec());
    let Some(_opts_guard) = crate::compile_options_guard::CompileOptionsGuard::new(mozjs::glue::NewCompileOptions(raw, c_filename.as_ptr(), 1) as *mut _) else {
        return ptr::null_mut();
    };
    ...
    let ok = mozjs_sys::jsapi::JS::Evaluate2(raw, opts as *const _, &mut src_text, rval_h);
