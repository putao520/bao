...
unsafe extern "C" fn cp_spawn(
    ...
    let spawn_result = match spawn_sync::spawn(&sync_opts) {
        Ok(Ok(r)) => r,
        Ok(Err(sys_err)) => {
            let c_msg = bun_core::ZBox::from_bytes(format!("spawn system error: {:?}", sys_err).as_bytes());
            JS_ReportErrorUTF8(cx, c"%s".as_ptr(), c_msg.as_ptr());
            return false;
        }
        Err(e) => {
            let c_msg = bun_core::ZBox::from_bytes(format!("spawn failed: {:?}", e).as_bytes());
            JS_ReportErrorUTF8(cx, c"%s".as_ptr(), c_msg.as_ptr());
            return false;
        }
    };
    ...
    let pid = bun_libuv_sys::uv_os_getpid();
...
unsafe extern "C" fn cp_exec(
    ...
    let spawn_result = match spawn_sync::spawn(&sync_opts) {
        Ok(Ok(r)) => r,
        Ok(Err(sys_err)) => {
            let c_msg = bun_core::ZBox::from_bytes(format!("exec system error: {:?}", sys_err).as_bytes());
            JS_ReportErrorUTF8(cx, c"%s".as_ptr(), c_msg.as_ptr());
            return false;
        }
        Err(e) => {
            let c_msg = bun_core::ZBox::from_bytes(format!("exec failed: {:?}", e).as_bytes());
            JS_ReportErrorUTF8(cx, c"%s".as_ptr(), c_msg.as_ptr());
            return false;
        }
    };
    ...
    {
        let c_stdout = bun_core::ZBox::from_bytes(stdout_str.as_str().as_bytes());
    ...
    {
        let c_stderr = bun_core::ZBox::from_bytes(stderr_str.as_str().as_bytes());
    ...
    if let Some(cb_obj) = callback {
        ...
        let err_obj = if exit_code != 0 {
            ...
            if !e.is_null() {
                let msg_zbox = {
                    let prefix = b"Command failed with exit code ";
                    let mut itoa_buf = bun_core::fmt::ItoaBuf::default();
                    let code_str = bun_core::fmt::itoa(&mut itoa_buf, exit_code);
                    let mut bytes = Vec::with_capacity(prefix.len() + code_str.len());
                    bytes.extend_from_slice(prefix);
                    bytes.extend_from_slice(code_str);
                    bun_core::ZBox::from_bytes(bytes)
                };
                let msg_str = JS_NewStringCopyZ(cx, msg_zbox.as_ptr());
                if !msg_str.is_null() {
...
unsafe extern "C" fn cp_exec_sync(
    ...
    let spawn_result = match spawn_sync::spawn(&sync_opts) {
        Ok(Ok(r)) => r,
        Ok(Err(sys_err)) => {
            let c_msg = bun_core::ZBox::from_bytes(format!("execSync system error: {:?}", sys_err).as_bytes());
            JS_ReportErrorUTF8(cx, c"%s".as_ptr(), c_msg.as_ptr());
            return false;
        }
        Err(e) => {
            let c_msg = bun_core::ZBox::from_bytes(format!("execSync failed: {:?}", e).as_bytes());
            JS_ReportErrorUTF8(cx, c"%s".as_ptr(), c_msg.as_ptr());
            return false;
        }
    };
    ...
    let c_out = bun_core::ZBox::from_bytes(stdout_str.as_str().as_bytes());
...
unsafe extern "C" fn cp_exec_file(
    ...
    let spawn_result = match spawn_sync::spawn(&sync_opts) {
        Ok(Ok(r)) => r,
        Ok(Err(sys_err)) => {
            let c_msg = bun_core::ZBox::from_bytes(format!("execFile system error: {:?}", sys_err).as_bytes());
            JS_ReportErrorUTF8(cx, c"%s".as_ptr(), c_msg.as_ptr());
            return false;
        }
        Err(e) => {
            let c_msg = bun_core::ZBox::from_bytes(format!("execFile failed: {:?}", e).as_bytes());
            JS_ReportErrorUTF8(cx, c"%s".as_ptr(), c_msg.as_ptr());
            return false;
        }
    };
    ...
    let stderr_str = String::from_utf8_lossy(&stderr_bytes).into_owned();
    {
        let c_err = bun_core::ZBox::from_bytes(stderr_str.as_str().as_bytes());
...
unsafe extern "C" fn cp_exec_file_sync(
    ...
    let spawn_result = match spawn_sync::spawn(&sync_opts) {
        Ok(Ok(r)) => r,
        Ok(Err(sys_err)) => {
            let c_msg = bun_core::ZBox::from_bytes(format!("execFileSync system error: {:?}", sys_err).as_bytes());
            JS_ReportErrorUTF8(cx, c"%s".as_ptr(), c_msg.as_ptr());
            return false;
        }
        Err(e) => {
            let c_msg = bun_core::ZBox::from_bytes(format!("execFileSync failed: {:?}", e).as_bytes());
            JS_ReportErrorUTF8(cx, c"%s".as_ptr(), c_msg.as_ptr());
            return false;
        }
    };
    ...
    if exit_code != 0 {
        ...
        let c_msg = bun_core::ZBox::from_bytes(format!("execFileSync failed with status {}: {}", exit_code, stderr_str).as_bytes());
            JS_ReportErrorUTF8(cx, c"%s".as_ptr(), c_msg.as_ptr());
        return false;
    }
    let stdout_str = String::from_utf8_lossy(&spawn_result.stdout).into_owned();
...
unsafe extern "C" fn cp_spawn_sync(
    ...
    let spawn_result = match spawn_sync::spawn(&sync_opts) {
        Ok(Ok(r)) => r,
        Ok(Err(sys_err)) => {
            let c_msg = bun_core::ZBox::from_bytes(format!("spawnSync system error: {:?}", sys_err).as_bytes());
            JS_ReportErrorUTF8(cx, c"%s".as_ptr(), c_msg.as_ptr());
            return false;
        }
        Err(e) => {
    ...
    let pid = Int32Value(bun_libuv_sys::uv_os_getpid() as i32);
...
unsafe extern "C" fn cp_fork(
    ...
    let spawn_result = match spawn_sync::spawn(&sync_opts) {
        Ok(Ok(r)) => r,
        Ok(Err(sys_err)) => {
            let c_msg = bun_core::ZBox::from_bytes(format!("fork system error: {:?}", sys_err).as_bytes());
            JS_ReportErrorUTF8(cx, c"%s".as_ptr(), c_msg.as_ptr());
            return false;
        }
        Err(e) => {
            let c_msg = bun_core::ZBox::from_bytes(format!("fork failed: {:?}", e).as_bytes());
            JS_ReportErrorUTF8(cx, c"%s".as_ptr(), c_msg.as_ptr());
            return false;
        }
    };
    ...
    let pid_v = Int32Value(bun_libuv_sys::uv_os_getpid() as i32);
    rooted!(&in(cx_ref) let pv = pid_v);
    JS_DefineProperty(cx, child_h, c"pid".as_ptr(), pv.handle().into(), JSPROP_ENUMERATE as u32);
