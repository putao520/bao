...
fn _force_native_stubs_link() {
    ...
    // Initialize socket dispatch vtables: Tier-1 (HTTPClient) first.
    bun_socket_dispatch::init_http_vtables();
    // JSC-bound vtables (BunSocket, Postgres, etc.) are NOT registered here —
    // they require bun_jsc which is incompatible with SpiderMonkey. SM-mode
    // socket handling uses bao_engine's own event loop path instead.
}
pub mod bun_api;
pub mod bun_test;
pub mod compile_options_guard;
pub mod dispatch;
pub mod fetch_api;
pub mod gc_store;
pub mod globals;
