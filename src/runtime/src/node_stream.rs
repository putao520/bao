...
pub fn install(cx: &mut mozjs::context::JSContext) {
    ...
    unsafe {
        ...
        let c_filename = c"node:stream".as_ptr();
        let Some(_opts_guard) = crate::compile_options_guard::CompileOptionsGuard::new(NewCompileOptions(cx_raw, c_filename, 1) as *mut _) else {
            return;
        };
