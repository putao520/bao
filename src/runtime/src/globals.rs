...
pub unsafe fn install_module_on_target(
  ...
  defineExports(g);
    ...
    let Some(_opts_guard) = crate::compile_options_guard::CompileOptionsGuard::new(mozjs::glue::NewCompileOptions(raw, c"<module-setup>".as_ptr(), 1) as *mut _) else { return; };
    let opts = _opts_guard.as_ptr();
    let ok = JS::Evaluate2(raw, opts as *const _, &mut setup_text, factory_h);
...
fn install_file_globals_from_cache(
    ...
    unsafe {
        ...
        if let Some(fn_str) = filename {
                let c_fn = bun_core::ZBox::from_bytes(fn_str.as_bytes());
        ...
        if let Some(dir_str) = dirname {
                let c_dir = bun_core::ZBox::from_bytes(dir_str.as_bytes());
...
"#;
    unsafe {
        let raw = cx.raw_cx();
        let Some(_opts_guard) = crate::compile_options_guard::CompileOptionsGuard::new(mozjs::glue::NewCompileOptions(raw, c"<buffer-proto>".as_ptr(), 1) as *mut _) else { return; };
        let opts = _opts_guard.as_ptr();
            ...
            mozjs_sys::jsapi::JS::Evaluate2(raw, opts as *const _, &mut src, rval_h);
    }
}
...
unsafe extern "C" fn buffer_from(
    ...
    if input.is_string() {
        ...
        let bytes = if encoding == "hex" {
            // 铁律0: use bun_core::string::decode_hex_to_bytes_truncate (SIMD highway)
            // instead of hand-written step_by(2) + u8::from_str_radix loop
            let src = s.as_bytes();
            let len = src.len() / 2;
...
unsafe extern "C" fn buffer_to_string(
    ...
    let c_s = bun_core::ZBox::from_bytes(output.as_bytes());
...
pub fn install_crypto_global(
    ...
    unsafe {
        ...
        JS_DefineFunction(cx, crypto_obj.handle(), c"randomUUID".as_ptr(), Some(crate::node_crypto::crypto_random_uuid), 0, JSPROP_ENUMERATE as u32);
        JS_DefineFunction(cx, crypto_obj.handle(), c"getRandomValues".as_ptr(), Some(crate::node_crypto::crypto_get_random_values), 1, JSPROP_ENUMERATE as u32);
...
unsafe extern "C" fn crypto_subtle_digest(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
    ...
    let hash = match algo.as_str() {
        "sha-1" | "sha1" => { let mut out = [0u8; 20]; hashers::SHA1::hash(&bytes, &mut out); out.to_vec() }
        "sha-256" | "sha256" => { let mut out = [0u8; 32]; hashers::SHA256::hash(&bytes, &mut out); out.to_vec() }
        "sha-384" | "sha384" => { let mut out = [0u8; 48]; hashers::SHA384::hash(&bytes, &mut out); out.to_vec() }
        "sha-512" | "sha512" => { let mut out = [0u8; 64]; hashers::SHA512::hash(&bytes, &mut out); out.to_vec() }
        _ => {
...
unsafe extern "C" fn structured_clone_fn(
    ...
    if val.is_object() {
            ...
            if name_val.is_string() {
                ...
                match name.as_str() {
                    "Date" => {
                        ...
                        if time_val.is_object() {
                            ...
                            if !global.is_null() {
                                ...
                                let Some(_eval_opts_guard) = crate::compile_options_guard::CompileOptionsGuard::new(mozjs::glue::NewCompileOptions(cx, c"clone".as_ptr(), 1) as *mut _) else {
                                    args.rval().set(eval_rval);
                                    return true;
                                };
                                let eval_opts = _eval_opts_guard.as_ptr();
                                    let mut src_text = mozjs::rust::transform_str_to_source_text(&src);
                                    JS::Evaluate2(cx, eval_opts as *const _, &mut src_text, MutableHandle::<Value> { _phantom_0: ::std::marker::PhantomData, ptr: &mut eval_rval });
                                args.rval().set(eval_rval);
                                return true;
                            }
                        }
                    ...
                    "RegExp" => {
                        ...
                        // D42: use bun_core JSON escaping instead of hand-written replace chain
                        let mut esc_source = String::new();
                        bun_core::fmt::encode_json_string_chars(&mut esc_source, source.as_bytes()).ok();
                        let mut esc_flags = String::new();
                        bun_core::fmt::encode_json_string_chars(&mut esc_flags, flags.as_bytes()).ok();
                        let src = format!("new RegExp(\"{}\", \"{}\")", esc_source, esc_flags);
                        let mut eval_rval = UndefinedValue();
                        let Some(_eval_opts_guard) = crate::compile_options_guard::CompileOptionsGuard::new(mozjs::glue::NewCompileOptions(cx, c"clone".as_ptr(), 1) as *mut _) else {
                            args.rval().set(eval_rval);
                            return true;
        ...
        let Some(_json_opts_guard) = crate::compile_options_guard::CompileOptionsGuard::new(mozjs::glue::NewCompileOptions(cx, c"json_clone".as_ptr(), 1) as *mut _) else { return false; };
        let json_opts = _json_opts_guard.as_ptr();
            let mut json_fn_val = UndefinedValue();
            JS::Evaluate2(cx, json_opts as *const _, &mut ::std::mem::MaybeUninit::new(json_src).assume_init(), MutableHandle::<Value> { _phantom_0: ::std::marker::PhantomData, ptr: &mut json_fn_val });
...
if (typeof _g.AbortController === 'undefined') {
  var _EE = null;
  try { _EE = require("events").EventEmitter; } catch(e) {
    _EE = function EE() { this._events = {}; };
    _EE.prototype.on = function(e, fn) { (this._events[e] || (this._events[e] = [])).push(fn); return this; };
    _EE.prototype.emit = function(e) { var a = Array.prototype.slice.call(arguments, 1); var ls = this._events[e]; if (ls) for (var i = 0; i < ls.length; i++) ls[i].apply(this, a); return !!ls; };
    _EE.prototype.removeListener = function(e, fn) { var ls = this._events[e]; if (ls) { var i = ls.indexOf(fn); if (i >= 0) ls.splice(i, 1); } return this; };
  }
  ...
  _g.AbortSignal.prototype = Object.create(_EE.prototype);
  _g.AbortSignal.prototype.constructor = _g.AbortSignal;
  _g.AbortSignal.prototype.addEventListener = function(type, fn) {
    if (type === 'abort') this.on('abort', fn);
  };
  _g.AbortSignal.prototype.removeEventListener = function(type, fn) {
    if (type === 'abort') this.removeListener('abort', fn);
  };
  _g.AbortController = function AbortController() {
    ...
    this.abort = function(reason) {
      ...
      signal.emit('abort', { type: 'abort', target: signal });
    };
  };
}
...
"#;
    unsafe {
        let raw = cx.raw_cx();
        let mut rval = UndefinedValue();
        let Some(_opts_guard) = crate::compile_options_guard::CompileOptionsGuard::new(mozjs::glue::NewCompileOptions(
            raw,
            c"web_api_constructors".as_ptr(),
            1,
        ) as *mut _) else { return; };
        let opts = _opts_guard.as_ptr();
            let mut src_text = mozjs::rust::transform_str_to_source_text(src);
            mozjs_sys::jsapi::JS::Evaluate2(raw, opts as *const _, &mut src_text, MutableHandle::<Value> { _phantom_0: ::std::marker::PhantomData, ptr: &mut rval });
    }
}
...
pub unsafe fn create_node_api_scope_values(
    ...
    let scope_name_c = bun_core::ZBox::from_bytes(scope_name.as_bytes());
    JS_DefineProperty3(
        cx,
        global,
        scope_name_c.as_ptr(),
