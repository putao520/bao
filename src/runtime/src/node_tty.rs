...
unsafe extern "C" fn tty_isatty(_cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
    ...
    let result = if fd >= 0 { bun_sys::isatty(bun_sys::Fd(fd)) as core::ffi::c_int } else { 0 };
...
unsafe extern "C" fn tty_read_stream_ctor(
    ...
    // isTTY
    rooted!(&in(cx_ref) let tv = BooleanValue(bun_sys::isatty(bun_sys::Fd(fd))));
...
unsafe extern "C" fn tty_write_stream_ctor(
    ...
    let is_tty = bun_sys::isatty(bun_sys::Fd(fd));

    // isTTY
    rooted!(&in(cx_ref) let tv = BooleanValue(is_tty));
    JS_DefineProperty(cx, obj.handle().into(), c"isTTY".as_ptr(), tv.handle().into(), JSPROP_ENUMERATE as u32);

    // columns / rows from TIOCGWINSZ
    let ws = bun_sys::tty_winsize(bun_sys::Fd(fd));
    if let Some(ws) = ws {
        rooted!(&in(cx_ref) let cols = Int32Value(ws.col as i32));
        JS_DefineProperty(cx, obj.handle().into(), c"columns".as_ptr(), cols.handle().into(), JSPROP_ENUMERATE as u32);
        rooted!(&in(cx_ref) let rows = Int32Value(ws.row as i32));
...
unsafe extern "C" fn tty_set_raw_mode(
    ...
    // D37: bun_core::tty::set_mode replaces libc::tcgetattr/cfmakeraw/tcsetattr
    let mode = if raw_flag {
        bun_core::tty::Mode::Raw
    } else {
        bun_core::tty::Mode::Normal
    };
    let rc = bun_core::tty::set_mode(fd, mode);
    if rc != 0 {
        JS_ReportErrorUTF8(cx, c"setRawMode: set_mode failed".as_ptr());
        return false;
    }
...
unsafe extern "C" fn tty_get_window_size(
    ...
    if let Some(ws) = ws {
        ...
        if !arr.get().is_null() {
            rooted!(&in(cx_ref) let cv = Int32Value(ws.col as i32));
            JS_DefineElement(cx, arr.handle().into(), 0, cv.handle().into(), JSPROP_ENUMERATE as u32);
            rooted!(&in(cx_ref) let rv = Int32Value(ws.row as i32));
...
unsafe extern "C" fn tty_write_stream_write(
    ...
    if argc > 0 {
        ...
        if v.is_string() {
            ...
            // 铁律0: use bun_sys::File instead of libc::write
            bun_sys::File::from_fd(bun_sys::Fd(fd)).write_all(s.as_bytes()).ok();
        }
    }
