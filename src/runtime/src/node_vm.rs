...
fn eval_code(cx: *mut JSContext, code: &str, filename: &str) -> Option<*mut JSObject> {
    unsafe {
        let c_filename = bun_core::ZBox::from_bytes(filename.as_bytes());
        let Some(_opts_guard) = crate::compile_options_guard::CompileOptionsGuard::new(mozjs::glue::NewCompileOptions(cx, c_filename.as_ptr() as *const _, 1) as *mut _) else {
            return None;
        };
...
unsafe extern "C" fn vm_create_context(
    ...
    if argc > 0 && (*args.get(0).ptr).is_object() {
        ...
        "#;
        let c_filename = c"<vm>".as_ptr();
        let Some(_opts_guard) = crate::compile_options_guard::CompileOptionsGuard::new(mozjs::glue::NewCompileOptions(cx, c_filename, 1) as *mut _) else {
