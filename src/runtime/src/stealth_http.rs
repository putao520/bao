...
mod tests {
    ...
    fn test_ssl_config_firefox_tls12_cipher_content() {
        let profile = StealthProfile::firefox_default();
        let config = stealth_profile_to_ssl_config(&Some(profile));
        let s = unsafe { bun_core::ZStr::from_c_ptr(config.tls12_cipher_list) }.to_str().unwrap();
    ...
    fn test_ssl_config_firefox_tls13_cipher_content() {
        let profile = StealthProfile::firefox_default();
        let config = stealth_profile_to_ssl_config(&Some(profile));
        let s = unsafe { bun_core::ZStr::from_c_ptr(config.tls13_cipher_suites) }.to_str().unwrap();
    ...
    fn test_ssl_config_firefox_curves_content() {
        let profile = StealthProfile::firefox_default();
        let config = stealth_profile_to_ssl_config(&Some(profile));
        let s = unsafe { bun_core::ZStr::from_c_ptr(config.tls_curves_list) }.to_str().unwrap();
    ...
    fn test_ssl_config_firefox_sigalgs_content() {
        let profile = StealthProfile::firefox_default();
        let config = stealth_profile_to_ssl_config(&Some(profile));
        let s = unsafe { bun_core::ZStr::from_c_ptr(config.tls_sigalgs_list) }.to_str().unwrap();
    ...
    fn test_ssl_config_firefox_chrome_different_ciphers() {
        ...
        let ff_s = unsafe { bun_core::ZStr::from_c_ptr(ff_config.tls12_cipher_list) }.to_str().unwrap();
        let ch_s = unsafe { bun_core::ZStr::from_c_ptr(ch_config.tls12_cipher_list) }.to_str().unwrap();
