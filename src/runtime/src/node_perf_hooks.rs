...
pub fn install(cx: &mut mozjs::context::JSContext) {
    ...
    unsafe {
        ...
        {
            let Some(_opts_guard) = crate::compile_options_guard::CompileOptionsGuard::new(NewCompileOptions(cx.raw_cx(), c"node:perf_hooks".as_ptr(), 1) as *mut _) else { return; };
            let opts = _opts_guard.as_ptr();
            let mut src_text = mozjs::rust::transform_str_to_source_text(shim_src);
            ...
            if ok && rval.is_object() {
                    ...
                    let mod_val = ObjectValue(perf_mod.get());
