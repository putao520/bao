...
fn test_stealth_http_deep() {
    ...
    let chrome_akamai = bun_runtime::stealth_http::akamai_fingerprint(&Some(chrome_profile.clone()));
    ...
    let ordered_chrome = bun_runtime::stealth_http::ordered_headers(&Some(chrome_profile.clone()), &pseudo_headers);
    ...
    let ordered_ff = bun_runtime::stealth_http::ordered_headers(&Some(ff_profile.clone()), &pseudo_headers);
