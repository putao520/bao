...
// ─── BoringSSL ABI compatibility stubs ──────────────────────────────────
//
// These functions are referenced by bun_http but not exported by the
// BoringSSL C++ library in this build. They are provided as no-ops or
// minimal implementations that bun_http expects.

/// SSL_set_ciphersuites — configure TLS 1.3 cipher suites.
///
/// Not exported by this BoringSSL build. Provided as a no-op stub that
/// returns success (1). BoringSSL's TLS 1.3 cipher suite configuration
/// is handled internally based on the cipher list string.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn SSL_set_ciphersuites(_ssl: *mut c_void, _str: *const u8) -> c_int {
    1
}
