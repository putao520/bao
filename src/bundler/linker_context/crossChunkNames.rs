...
pub(crate) fn apply_to_clauses(c: &LinkerContext, chunks: &mut [Chunk]) {
    ...
    for chunk in chunks.iter_mut() {
            ...
            .chain(js.cross_chunk_prefix_stmts.iter_mut())
        {
            ...
            for item in items {
                // PORT NOTE: bao's `LocRef.ref_` is `Option<Ref>` (upstream:
                // plain `Ref`); the linker-generated cross-chunk clauses
                // always carry a concrete ref.
                let ref_ = item.name.ref_.expect("cross-chunk clause item ref");
                item.alias = bun_ast::StoreStr::new(*c.cross_chunk_names.get(&ref_).unwrap());
            }
        }
    }
}
