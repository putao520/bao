...
use crate::{Index, LinkerContext, WrapKind};
...
pub(crate) fn merge_small_chunks(
    ...
    if !fold_pure
        && !entry_source_indices
            .iter()
            .any(|&source_index| kinds[source_index as usize] == crate::EntryPointKind::DynamicImport)
    {
        return Ok(());
    }
    ...
    let is_dynamic_entry = |entry_id: usize| {
        kinds[entry_source_indices[entry_id] as usize] == crate::EntryPointKind::DynamicImport
    };
    ...
    let pin_entry_chunk = |entry_id: usize| {
        ...
        // PORT NOTE: bao's `LinkerOptions` carries `compile: bool` where
        // upstream has `CompileMode::is_executable()`.
        (this.options.compile && !is_dynamic_entry(entry_id))
    ...
    for source_index in this.graph.reachable_files.iter() {
        ...
        // PORT NOTE: bao's `VacantEntry` has no `index()`; a vacant entry
        // appends at the map's current length, so capture it before `entry()`
        // borrows the map.
        let vacant_index = groups.count();
        ...
        let group_index = match &entry {
            ...
            MapEntry::Vacant(_) => vacant_index,
        };
