        ...
        _ => {
            ...
            if let ExprData::EObject(e_object) = &expr.data {
                for property in e_object.properties.slice() {
                    ...
                    // upstream 11e0f96b65: flatten into a local copy — never resolve the
                    // rope in place through `DerefMut` into the shared arena slot.
                    ...
                    // upstream 11e0f96b65: read-only flatten (`string()` instead of the
                    // in-place `slice()`).
