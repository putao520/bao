...
pub fn generate_chunks_in_parallel<const IS_DEV_SERVER: bool>(
...

    {
        ...
        if c.parse_graph().css_file_count > 0 {
            ...
            // Per-batch group: waits for *this* batch only, not for every task on
            // the shared pool (an unrelated fs/crypto task blocking a worker must
            // not stall the link step).
            let group = WaitGroup::init_with_count(total_count);
...

    {
...

        {
            ...
            // Per-batch group (see the CSS AST batch above).
            let group = WaitGroup::init_with_count(total_count);
    ...
    // TODO: enforceNoCyclicChunkImports()
    {
        ...
        for index in 0..chunks.len() {
            ...
            if strings::index_of(&rel_path, b"/./").is_some() {
                ...
                rel_path = path::resolve_path::normalize_buf::<path::platform::Posix>(
                    &rel_path,
                    &mut buf,
                )
                .to_vec();
                        ...
                        if let Some(bytecode) = crate::bundle_v2::dispatch::generate_cached_bytecode(
                            ...
                            break 'brk Some(options::OutputFile::init(options::OutputFileInit {
                                ...
                                hash: chunk
                                    ...
                                    .placeholder
                                    .hash
                                    .map(|_| chunk.template.content_hash(bun_wyhash::hash(&bytecode))),
    ...
    // f039a9c563
    if is_compile {
        for (chunk_index, &position) in chunk_load_order(c, chunks, &output_files.output_files)
            .iter()
            .enumerate()
        {
...
fn chunk_load_order(
    ...
    let target_is_bun = c.options.target.is_bun();
