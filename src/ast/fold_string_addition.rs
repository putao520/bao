...
// upstream 14a4f45c54: inlined enum members are stored flat (s_enum resolves
// the rope before storing the EnumString), so `join_strings` no longer needs
// `has_inlined_enum_poison` / `clone_rope_nodes` — linking in O(1) is sound.
