...
impl<'a> URL<'a> {
    ...
    /// Upstream bun 63a495cb46 (B10): the `path` of a connection error and
    /// the cross-origin redirect rule leave the userinfo out.
...
/// `[::1]` -> `::1`. Anything else in brackets, such as `[example.com]`,
/// passes through verbatim, as in Node.
///
/// Upstream bun 63a495cb46 (B3) moved this to `bun_url` (re-exported from
/// `bun_http`) so every consumer — TLS hostname/SNI, HTTP/3 dial, NO_PROXY
/// entries — strips at one chokepoint. bao's carrier is `bun_core`'s
/// pton-based `is_ipv6_address` (upstream's `ip_address` module is not
/// ported).
pub fn strip_ipv6_brackets(host: &[u8]) -> &[u8] {
    if host.len() >= 2 && host[0] == b'[' && host[host.len() - 1] == b']' {
        let inner = &host[1..host.len() - 1];
        if strings::is_ipv6_address(inner) {
            return inner;
        }
    }
    host
...
/// Upstream 66c669c962: the fragment starts at the first `#`, so a `?`
/// after it is part of the fragment, not the start of the query. The
/// request-target written by the HTTP client is `pathname + search`; the
/// fragment (even one containing a `?`) never reaches the wire.
#[cfg(test)]
mod fragment_query_tests {
    use super::URL;
