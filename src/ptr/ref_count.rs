...
mod tests {
    ...
    // Link-guidance only — no test semantics: product code under test reaches
    // `scoped_log!`, which dispatches through bun_core's `OutputSink[Sys]`
    // link-interface (`__bun_dispatch__OutputSink__Sys__*`, declared as
    // `unsafe extern "Rust"` in bun_core). The Sys-arm definitions are emitted
    // by `link_impl_OutputSink!` in bun_sys; rustc drops an unreferenced extern
    // crate from the test link line, so this use forces the bun_sys rlib onto
    // it and the undefined dispatch symbols resolve.
