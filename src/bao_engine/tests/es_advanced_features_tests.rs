...
fn test_es_advanced_features() {
    ...
    assert!(
        ...
        "Reflect.ownKeys should list keys, got: {}",
        result
    );
    ...
    assert!(
        result.contains("string") || result.contains("number") || result.contains("default"),
        "Symbol.toPrimitive should be called, got: {}",
        result
    );
    ...
    assert_eq!(
        result, "a,x,y,b",
        "yield* should delegate to inner generator"
    );
    ...
    assert_eq!(
        result2, "fallback",
        "Optional chaining with null should work"
    );

    // Nullish coalescing
    let result3 = eval_string(
        &mut ctx,
        r#"
    ...
    assert_eq!(
        result3, "default|default|0|",
        "Nullish coalescing should only trigger for null/undefined"
    );
