...
fn test_error_handling_deep() {
    ...
    assert!(
        result.contains("RangeError"),
        "negative Array length should throw RangeError, got: {}",
        result
    );
    ...
    assert_eq!(
        ...
        "undeclared variable should throw ReferenceError"
    );
    ...
    assert!(
        result.contains("Error") && result.contains("test"),
        "Error toString should contain type and message, got: {}",
        result
    );
