...
pub fn generate_bindings(class_def: &ClassDef) -> GeneratedBindings {
    ...
    !proto.is_null()
        ...
        class_name = class_name,
        js_name = js_name,
        constructor_ref = if constructor_fn.is_some() {
            format!("Some({class_name}_constructor)")
        } else {
            "None".into()
        },
        proto_array = format!("{class_name}_proto_specs"),
        static_array = format!("{class_name}_static_specs"),
...
pub fn generate_module(bindings: &[GeneratedBindings], module_name: &str) -> String {
    ...
    out.push_str(&format!("// @trace REQ-ENG-002 — Auto-generated by bao_engine codegen\n"));
