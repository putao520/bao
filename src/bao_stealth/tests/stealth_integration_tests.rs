...
#[test]
fn test_chrome_js_contains_user_agent() {
    let engine = StealthEngine::new(StealthProfile::chrome_default());
    let js = engine.inject_navigator_js();
    assert!(js.contains("userAgent"), "JS should override userAgent");
    assert!(js.contains("navigator"), "JS should reference navigator");
}

#[test]
fn test_firefox_js_contains_platform() {
    let engine = StealthEngine::new(StealthProfile::firefox_default());
    let js = engine.inject_navigator_js();
    assert!(js.contains("platform"), "JS should override platform");
    assert!(js.contains("language"), "JS should override language");
}

#[test]
fn test_js_contains_screen_overrides() {
    let engine = StealthEngine::new(StealthProfile::chrome_default());
    let js = engine.inject_navigator_js();
    assert!(js.contains("screen"), "JS should override screen properties");
    assert!(js.contains("availWidth"), "JS should set availWidth");
    assert!(js.contains("availHeight"), "JS should set availHeight");
    assert!(js.contains("devicePixelRatio"), "JS should set devicePixelRatio");
}

#[test]
fn test_js_contains_webgl_override() {
    let engine = StealthEngine::new(StealthProfile::chrome_default());
    let js = engine.inject_navigator_js();
    assert!(js.contains("WebGLRenderingContext"), "JS should override WebGL");
    assert!(js.contains("getParameter"), "JS should hook getParameter");
}

#[test]
fn test_js_contains_cdp_stealth() {
    let engine = StealthEngine::new(StealthProfile::chrome_default());
    let js = engine.inject_navigator_js();
    assert!(js.contains("cdc_adoQpoasnfa76pfcZLmcfl"), "JS should remove ChromeDriver markers");
    assert!(js.contains("window.chrome"), "JS should clean window.chrome");
}

#[test]
fn test_js_contains_webdriver_false() {
    let engine = StealthEngine::new(StealthProfile::chrome_default());
    let js = engine.inject_navigator_js();
    assert!(js.contains("webdriver"), "JS should override webdriver");
    assert!(js.contains("false"), "webdriver should be set to false");
}

#[test]
fn test_js_contains_hardware_concurrency() {
    let engine = StealthEngine::new(StealthProfile::chrome_default());
    let js = engine.inject_navigator_js();
    assert!(js.contains("hardwareConcurrency"), "JS should set hardwareConcurrency");
}

#[test]
fn test_js_contains_max_touch_points() {
    let engine = StealthEngine::new(StealthProfile::chrome_default());
    let js = engine.inject_navigator_js();
    assert!(js.contains("maxTouchPoints"), "JS should set maxTouchPoints");
}
