...
fn test_behavior_simulator_cross_thread_consistency() {
    ...
    for i in 1..results.len() {
        assert_eq!(
            ...
            "Mouse path differs across threads"
        );
        assert_eq!(
            results[0].1, results[i].1,
            "Typing delays differ across threads"
        );
        assert_eq!(
            results[0].2, results[i].2,
            "Scroll deltas differ across threads"
        );
    }
}
    ...
    let handles: Vec<_> = (0..8)
        .map(|_| {
            thread::spawn(|| {
                ...
                (0..10)
                    .map(|i| noise.apply_to_pixel(128, 128, 128, 255, i, i))
                    .collect::<Vec<_>>()
            })
        })
        .collect();
...
fn test_canvas_noise_different_seeds_isolation() {
    let handles: Vec<_> = (0..8)
        .map(|seed| {
            thread::spawn(move || {
                ...
                (0..5)
                    .map(|i| noise.apply_to_pixel(100, 100, 100, 255, i, i))
                    .collect::<Vec<_>>()
            })
        })
        .collect();
    ...
    for i in 1..results.len() {
        assert_ne!(
            results[0], results[i],
            "Different seeds produced same canvas noise"
        );
    }
}
...
fn test_behavior_simulator_intra_thread_consistency() {
    ...
    assert_eq!(
        path1, path2,
        "Same thread, same seed should produce identical paths"
    );
}
...
fn test_canvas_noise_intra_thread_consistency() {
    ...
    assert_eq!(
        p1, p2,
        "Same thread, same seed should produce identical pixel noise"
    );
}
