...
fn anti_crawler_detection_all() {
    ...
    report.check(
        "sannysoft::webdriver_type_boolean",
        webdriver_type == "boolean",
    ...
    if ua.contains("Firefox") {
        report.check(
            "sannysoft::firefox_no_chrome_object",
            !chrome_exists,
    ...
    report.check(
        "sannysoft::permissions_api_valid",
    ...
    report.check(
        "sannysoft::connection_type_valid",
    ...
    report.check(
        "sannysoft::ua_no_bot",
        !ua.to_lowercase().contains("bot"),
        &format!("UA contains 'bot': {}", ua),
    );
    report.check(
        "sannysoft::ua_no_crawler",
        !ua.to_lowercase().contains("crawler"),
        &format!("UA contains 'crawler': {}", ua),
    );
    report.check(
        "sannysoft::ua_no_spider",
        !ua.to_lowercase().contains("spider"),
        &format!("UA contains 'spider': {}", ua),
    );
    report.check(
        "sannysoft::ua_no_phantom",
        !ua.to_lowercase().contains("phantom"),
    ...
    report.check(
        "sannysoft::platform_not_empty",
        !platform.is_empty(),
        "navigator.platform is empty",
    );
    report.check(
        "sannysoft::ua_platform_match",
    ...
    report.check(
        "sannysoft::language_not_empty",
        !lang.is_empty(),
        "navigator.language is empty",
    );
    report.check(
        "sannysoft::language_valid_format",
    ...
    report.check(
        "sannysoft::screen_reasonable_size",
        sw >= 1024 && sh >= 600,
    ...
    report.check(
        "sannysoft::color_depth_not_zero",
        cd > 0,
    ...
    report.check(
        "sannysoft::dpr_not_zero",
        dpr > 0.0,
        "devicePixelRatio is zero or negative",
    );
    report.check(
        "sannysoft::dpr_reasonable",
        dpr >= 0.5 && dpr <= 4.0,
    ...
    report.check(
        "creepjs::ua_platform_consistent",
        (ua.contains("Linux") && platform.contains("Linux"))
            || (ua.contains("Win") && platform.contains("Win")),
    ...
    report.check(
        "creepjs::webdriver_is_false_string",
        webdriver_val == "false",
        &format!(
            "navigator.webdriver string is '{}', expected 'false'",
            webdriver_val
        ),
    );
    ...
    if is_firefox {
        report.check(
            "pixelscan::firefox_vendor_empty",
            vendor.is_empty(),
    ...
    if is_chrome {
        report.check(
            "pixelscan::chrome_vendor_google",
            vendor == "Google Inc.",
    ...
    let is_common = common_resolutions
        .iter()
        .any(|(cw, ch)| sw == *cw && sh == *ch);
    report.check(
        "pixelscan::screen_common_or_reasonable",
    ...
    report.check(
        "pixelscan::webgl_vendor_not_empty",
        !engine.webgl().vendor.is_empty(),
        "WebGL vendor is empty",
    );
    report.check(
        "pixelscan::webgl_renderer_not_empty",
        !engine.webgl().renderer.is_empty(),
        "WebGL renderer is empty",
    );
    report.check(
        "pixelscan::webgl_extensions_available",
        !engine.webgl().extensions.is_empty(),
    ...
    report.check(
        "pixelscan::canvas_noise_subtle",
        noise_r <= 2 && noise_g <= 2 && noise_b <= 2,
        &format!(
            "Canvas noise too visible: delta ({}, {}, {})",
    ...
    report.check(
        "recaptcha::no_automation_globals",
        !has_automation,
    ...
    report.check(
        "recaptcha::common_browser_os_combo",
        is_firefox_linux
            || is_chrome_linux
            || is_firefox_mac
            || is_chrome_mac
            ...
            || is_firefox_win,
        &format!(
            "Unusual browser/OS combo: UA='{}', platform='{}'",
            ua, platform
        ),
    );

    report.check(
        "recaptcha::screen_reasonable",
        sw >= 320 && sh >= 240,
        &format!("Screen {}x{} too small for real browser", sw, sh),
    );

    report.check(
        "recaptcha::hwc_reasonable",
        hwc >= 1 && hwc <= 32,
    ...
    if dm_type == "number" {
        ...
        report.check(
            "recaptcha::device_memory_reasonable",
            dm >= 2.0 && dm <= 16.0,
        ...
        report.check(
            "recaptcha::device_memory_reasonable",
            true,
            "not defined (acceptable)",
        );
    }

    report.check(
        "recaptcha::language_set",
        !lang.is_empty(),
        "navigator.language is empty — bot indicator",
    );

    report.check(
        "recaptcha::touch_consistent_with_screen",
        touch == 0,
        &format!(
            "maxTouchPoints {} is inconsistent with desktop screen",
            touch
        ),
    );

    report.check(
        "recaptcha::ua_no_headless",
        !ua.contains("Headless"),
        "UA contains 'Headless' — bot indicator",
    );

    report.check(
        "recaptcha::color_depth_standard",
        cd == 24 || cd == 32,
    ...
    let perf_exists2 = bool_eval(
        &mut ctx,
        "typeof performance !== 'undefined' && typeof performance.now === 'function'",
    );
    report.check(
        "cf::performance_now_exists",
        perf_exists2,
    ...
    report.check(
        ...
        ua_before == ua_after,
        &format!(
            "navigator.userAgent was overridden: '{}' -> '{}'",
            ua_before, ua_after
        ),
    );
    ...
    report.check(
        "descriptor::platform_not_overridable",
        plat_before == plat_after,
        &format!(
            "navigator.platform was overridden: '{}' -> '{}'",
            plat_before, plat_after
        ),
    );
    ...
    report.check(
        "descriptor::screen_width_not_overridable",
        sw_before == sw_after_val,
        &format!(
            "screen.width was overridden: {} -> {}",
            sw_before, sw_after_val
        ),
    );
    ...
    report.check(
        "descriptor::dpr_not_overridable",
        (dpr_before - dpr_after).abs() < f64::EPSILON,
        &format!(
            "devicePixelRatio was overridden: {} -> {}",
            dpr_before, dpr_after
        ),
    );
    ...
    report.check(
        "descriptor::webdriver_stable_reads",
    ...
    report.check(
        "descriptor::useragent_stable_reads",
        ua_s1 == ua_s2,
    ...
    report.check(
        "hw::hwc_defined",
        hwc > 0,
        "hardwareConcurrency is 0 or undefined",
    );
    report.check(
        "hw::hwc_power_of_2_or_common",
    ...
    report.check(
        "hw::avail_le_total_width",
        aw <= sw,
        &format!("availWidth {} > width {}", aw, sw),
    );
    report.check(
        "hw::avail_le_total_height",
        ah <= sh,
    ...
    report.check(
        "hw::aspect_ratio_reasonable",
        aspect >= 1.2 && aspect <= 2.5,
    ...
    report.check(
        "hw::dpr_positive",
        dpr > 0.0,
        "devicePixelRatio is not positive",
    );
    ...
    assert_eq!(
        fails, 0,
        "{} anti-crawler detection assertions FAILED",
        fails
    );
    ...
    for prefix in &mandatory_prefixes {
        ...
        assert!(
            !is_fail,
            "MANDATORY assertion '{}' FAILED — anti-crawler detection compromised!",
            prefix
        );
    }
...
fn cross_profile_isolation_pure_data() {
    ...
    assert_ne!(
        ff_audio, ch_audio,
        "Audio noise must differ between profiles"
    );
}
