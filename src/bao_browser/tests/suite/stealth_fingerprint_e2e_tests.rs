...
fn test_firefox_profile_screen_dimensions() {
    ...
    assert_eq!(
        ...
        "Firefox screen.width should be 1920"
    );
    assert_eq!(
        profile.screen.height, 1080,
        "Firefox screen.height should be 1080"
    );
    assert_eq!(
        profile.screen.avail_width, 1920,
        "Firefox screen.avail_width should be 1920"
    );
    assert_eq!(
        profile.screen.avail_height, 1040,
        "Firefox screen.avail_height should be 1040"
    );
    assert_eq!(
        profile.screen.color_depth, 24,
        "Firefox screen.color_depth should be 24"
    );
}
...
fn test_chrome_profile_screen_dimensions() {
    ...
    assert_eq!(
        profile.screen.width, 1920,
        "Chrome screen.width should be 1920"
    );
    assert_eq!(
        profile.screen.height, 1080,
        "Chrome screen.height should be 1080"
    );
    assert_eq!(
        profile.screen.color_depth, 24,
        "Chrome screen.color_depth should be 24"
    );
    assert_eq!(
        profile.screen.device_pixel_ratio, 1.0,
        "Chrome screen.device_pixel_ratio should be 1.0"
    );
}
...
fn test_set_profile_webdriver_always_false() {
    ...
    assert_eq!(
        firefox.navigator.max_touch_points, 0,
        "Desktop profile should have max_touch_points=0"
    );

    let chrome = StealthProfile::chrome_default();
    bao_stealth::engine_props::set_profile(&chrome);
    assert_eq!(
        chrome.navigator.max_touch_points, 0,
        "Desktop profile should have max_touch_points=0"
    );
}
...
fn test_set_profile_overwrites_on_switch() {
    ...
    assert_eq!(
        engine.navigator().vendor,
        "",
        "Firefox vendor should be empty"
    );
}
...
fn test_set_profile_chrome_vs_firefox_different() {
    ...
    assert_ne!(
        ff_engine.navigator().vendor,
        ch_engine.navigator().vendor,
        "Firefox and Chrome engines must have different vendor strings"
    );
    assert_ne!(
        ff_engine.webgl().vendor,
        ch_engine.webgl().vendor,
...
fn test_stealth_engine_custom_profile_stored() {
    ...
    assert_eq!(
        engine.profile().navigator.user_agent,
        chrome.navigator.user_agent,
        "Custom Chrome profile must be stored in engine"
    );
    assert_eq!(
        engine.profile().tls.ja3_hash,
        chrome.tls.ja3_hash,
...
fn test_stealth_engine_tls_config_matches_profile() {
    ...
    assert_eq!(
        engine.tls_config().cipher_suites,
        engine.profile().tls.cipher_suites,
...
fn test_stealth_engine_http2_config_matches_profile() {
    ...
    assert_eq!(
        engine.http2_config().initial_window_size,
        engine.profile().http2.initial_window_size,
        "Engine http2_config() initial_window_size must match profile"
    );
    assert_eq!(
        engine.http2_config().pseudo_header_order,
        engine.profile().http2.pseudo_header_order,
...
fn test_stealth_engine_navigator_matches_profile() {
    ...
    assert_eq!(
        engine.navigator().platform,
        engine.profile().navigator.platform
    );
    assert_eq!(engine.navigator().vendor, engine.profile().navigator.vendor);
    assert_eq!(
        engine.navigator().hardware_concurrency,
        engine.profile().navigator.hardware_concurrency
    );
}
...
fn test_stealth_engine_webgl_matches_profile() {
    ...
    assert_eq!(
        engine.webgl().vendor,
        engine.profile().webgl.vendor,
        "Engine webgl() vendor must match profile"
    );
    assert_eq!(
        engine.webgl().renderer,
        engine.profile().webgl.renderer,
        "Engine webgl() renderer must match profile"
    );
    assert_eq!(
        engine.webgl().extensions,
        engine.profile().webgl.extensions,
...
fn test_tls_fingerprint_config_ciphers_not_empty() {
    ...
    assert!(
        !config.tls12_cipher_list.is_empty(),
        "TLS 1.2 cipher list must not be empty"
    );
    assert!(
        !config.tls13_cipher_suites.is_empty(),
        "TLS 1.3 cipher suites must not be empty"
    );
    assert!(
        !config.curves_list.is_empty(),
        "Curves list must not be empty"
    );
    assert!(
        !config.sigalgs_list.is_empty(),
        "Signature algorithms list must not be empty"
    );
...
fn test_firefox_http2_header_table_size_positive() {
    ...
    assert_eq!(
        profile.http2.header_table_size, 65536,
        "Firefox header_table_size should be 65536"
    );
}
...
fn test_chrome_http2_header_table_size_positive() {
    ...
    assert_eq!(
        profile.http2.header_table_size, 65536,
        "Chrome header_table_size should be 65536"
    );
}
...
fn test_http2_fingerprint_has_settings_frame_params() {
    ...
    assert_eq!(
        payload.len(),
        6,
        "Settings frame payload must have 6 parameter tuples"
    );
    ...
    assert_eq!(
        parts.len(),
        6,
        "Akamai fingerprint must be 6 colon-separated fields"
    );

    let chrome = StealthProfile::chrome_default();
    let akamai_chrome = chrome.http2.akamai_fingerprint();
    assert_ne!(
        akamai, akamai_chrome,
        "Firefox and Chrome must have different Akamai fingerprints"
    );
}
...
fn test_full_chain_chrome_profile_consistency() {
    ...
    assert!(
        engine.tls_config().cipher_suites.len()
            < StealthProfile::firefox_default().tls.cipher_suites.len()
    );
...
fn test_full_chain_tls_config_boringssl_strings() {
    ...
    assert!(ff_config
        .tls12_cipher_list
        .contains("ECDHE-ECDSA-AES128-GCM-SHA256"));
    assert!(ff_config
        .tls12_cipher_list
    ...
    // bun_runtime stealth_http's converge test). Each list equals its own
    // profile's DHE-filtered set; differentiation survives via offered groups
    // (Firefox keeps P-521) and JA3/JA4.
    assert_eq!(
        ff_config.tls12_cipher_list, ch_config.tls12_cipher_list,
        "Firefox/Chrome TLS 1.2 lists must converge after DHE filtering"
    );
    assert_eq!(
        ff_config.tls12_cipher_list,
        firefox.tls.tls12_cipher_list_string(),
        "exposed list must equal Firefox's DHE-filtered set"
    );
    assert_eq!(
        ch_config.tls12_cipher_list,
        chrome.tls.tls12_cipher_list_string(),
        "exposed list must equal Chrome's DHE-filtered set"
    );
    assert_ne!(
        ff_config.curves_list, ch_config.curves_list,
        "Firefox/Chrome curves must differ (Firefox keeps P-521)"
    );
    assert!(ff_config.curves_list.contains("P-521"));
    assert!(!ch_config.curves_list.contains("P-521"));
    assert_ne!(
        firefox.tls.compute_ja4(),
        chrome.tls.compute_ja4(),
...
fn test_full_chain_compute_ja3_ja4_consistency() {
    ...
    assert!(
        computed_ja3.starts_with("771,"),
        "Computed JA3 must start with TLS version 771"
    );
    ...
    assert_ne!(
        computed_ja3, chrome_ja3,
        "Firefox and Chrome must have different computed JA3"
    );
}
...
fn test_full_chain_canvas_noise_deterministic() {
    ...
    assert_ne!(
        p1, p3,
        "Firefox and Chrome canvas noise should differ (different seeds)"
    );
}
...
fn test_full_chain_behavior_simulator_deterministic() {
    ...
    let path1 = firefox
        .behavior
        .generate_mouse_path(0.0, 0.0, 100.0, 100.0, 10);
    let path2 = firefox
        .behavior
        .generate_mouse_path(0.0, 0.0, 100.0, 100.0, 10);
    assert_eq!(
        path1, path2,
        "Mouse path must be deterministic for same seed"
    );
    ...
    assert_eq!(
        delays1, delays2,
        "Typing delays must be deterministic for same seed"
    );
    ...
    assert_ne!(
        path1, chrome_path,
        "Firefox and Chrome mouse paths should differ (different seeds)"
    );
}
...
fn test_stealth_props_injected_all() {
    ...
    assert!(
        ua.contains("Firefox"),
        "navigator.userAgent must contain Firefox (default profile): {}",
        ua
    );
    ...
    assert_eq!(
        vendor, "",
        "Firefox navigator.vendor must be empty: {}",
        vendor
    );
    ...
    assert!(
        ...
        "navigator.platform must contain Linux: {}",
        platform
    );
