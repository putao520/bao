...
pub struct NamesIterator<'a> {
    ...
    pub buf: bun_paths::path_buffer_pool::Guard,
...
impl<'a> Linker<'a> {
    ...
    pub fn build_target_package_dir(&mut self) -> Option<usize> {
        // SAFETY: `target_node_modules_path` is set at construction to either
        // a caller-owned `AbsPath` or the same buffer as `node_modules_path`;
        // both outlive `self` and are not mutated for the duration of this
        // read (mirrors Zig's aliasing `*AbsPath`).
    ...
    pub fn build_destination_dir(&mut self, global: bool) -> Option<usize> {
    ...
    pub fn link(&mut self, global: bool) {
        let (Some(package_dir_len), Some(mut dest_off)) = (
            ...
            // PORT NOTE: upstream `Error::Sys(ENAMETOOLONG)`; Bao's interned
            // Error has no errno arm — `err!("NameTooLong")` is the same file's
            // existing idiom for the bin-name bound checks below.
    ...
    pub fn unlink(&mut self, global: bool) {
        let (Some(package_dir_len), Some(mut dest_off)) = (
            ...
            // PORT NOTE: see `link()` — ENAMETOOLONG mapped to the interned
            // NameTooLong error, matching the bin-name bound checks below.
            self.err = Some(bun_core::err!("NameTooLong"));
            return;
        };
