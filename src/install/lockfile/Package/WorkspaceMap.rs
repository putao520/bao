...
impl WorkspaceMap {
    ...
    pub(crate) fn mark_self_contained<'l>(&mut self, list: &[&'l [u8]]) -> Vec<&'l [u8]> {
        // Dual-mode key mirror (nightly std `Box` / stable `AllocBox`): copy
        // through a plain deref so both build cfgs share one body.
...
fn process_workspace_name(
    ...
    // upstream 57b61eb8f3: `"installConfig": { "hoistingLimits": "workspaces" }`
