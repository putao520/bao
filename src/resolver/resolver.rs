...
// (same pattern as `__bun_regex_*`). `install` is the
...
fn bufs_storage_init() -> *mut Bufs {
    ...
    // all-zero bit-pattern is a valid `Bufs`. Zero-allocating (never leaving
    // the box unwritten) because integers must be initialized: a never-written
    // `[u8; N]` is UB even though every bit pattern is a valid `u8`. Runs once
    // per thread; `alloc_zeroed` of ~100 KiB is usually fresh OS-zeroed pages.
