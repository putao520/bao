...
/// every caller in-tree is one of these. (upstream 475b885d6e: the
/// `ZERO_VALUE`/`MAX` associated consts had no reader — `from_u32` is what
/// the decoders use — and are deleted.)
///
...
pub fn to_utf8_list_with_type_bun<const SKIP_TRAILING_REPLACEMENT: bool>(
    ...
    while let Some(i) = first_non_ascii16(utf16_remaining) {
        ...
        // upstream db059d8ff0: try_reserve_exact throws OutOfMemory instead of
        // aborting; the old `(i + count + list.len() + extra) - list.len()` folds
        // to `i + count + extra`.
    ...
    if !utf16_remaining.is_empty() {
        // upstream db059d8ff0: fallible reserve — the Err arm is now reachable.
