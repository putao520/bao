...
impl Fd {
    ...
    /// The backing width follows `FdBacking` (u64 windows / i32 posix) —
    /// slots are usize-wide either way.
    #[inline]
    pub const fn to_bits(self) -> FdBacking {
        self.0
    }
    /// Inverse of [`Fd::to_bits`].
    #[inline]
    pub const fn from_bits(v: FdBacking) -> Fd {
        Fd(v)
    }
...
pub fn getcwd(buf: &mut PathBuffer) -> Result<&ZStr, crate::Error> {
    ...
    unsafe {
        ...
        if p.is_null() {
            ...
            // `std.posix.getcwd` mapped ENOENT (cwd deleted/unlinked) to
            // `error.CurrentWorkingDirectoryUnlinked`; keep that name so the
            // root-error handler can print its dedicated hint instead of the
            // generic ENOENT fallback (upstream a2496de44).
