...
///
/// Every `pub` entry point that can leave the session released (socket events,
/// the registry / pool hand-offs in `HTTPContext::connect`, the per-request
/// wakeups from `HTTPThread`) takes one of these instead of `&mut self` and
/// goes through [`ClientSession::enter`], so the releases happen through the
/// holder's pointer after the body's `&mut` borrow has ended. Callers that need
/// the session alive across two entry points hold a
...
impl ClientSession {
    ...
    /// every stream fails and the socket ext's ref is released; unless a caller
    /// holds its own guard, the session is freed before this returns.
