...
pub fn install(cx: &mut mozjs::context::JSContext) {
    // Full punycode implementation (RFC 3492) as JS source — VERBATIM
    // transcription of npm punycode@2.1.0 (the same package Bun's punycode.ts
    // ports), wrapped in an IIFE. Kept character-for-character identical to
    // the canonical algorithm: the previous in-tree variant used the pre-2.0
    // `adapt()` formulas (threshold `floor((maxInt - tMin) * tMax) / 2` ≈ 2.7e9
    // and `/tMax` stepping instead of the 2.x `baseMinusTMin * tMax >> 1` = 455
    // and `/baseMinusTMin` stepping) — the bias never adapted, so encode()
    // emitted garbage digit runs and toASCII() never produced valid xn--
    // labels. The regexes are ASCII-only escapes (\u3002 etc.) so the source
    // carries no non-ASCII bytes.
