...
unsafe fn get_string_arg(
    ...
    NonNull::new(ptr).map(|nn| mozjs::conversions::jsstr_to_string(&mozjs::context::JSContext::from_ptr(NonNull::new_unchecked(cx)), nn))
}
...
unsafe fn parse_hash_options(cx: *mut JSContext, args: &CallArgs, argc: u32) -> HashOptions {
    ...
    if val.is_string() {
        ...
        if let Some(nn) = NonNull::new(ptr) {
            opts.algorithm = mozjs::conversions::jsstr_to_string(&mozjs::context::JSContext::from_ptr(NonNull::new_unchecked(cx)), nn);
        }
        return opts;
    }
...
unsafe fn parse_verify_options(cx: *mut JSContext, args: &CallArgs, argc: u32) -> Option<String> {
    ...
    if val.is_string() {
        let ptr = val.to_string();
        return NonNull::new(ptr).map(|nn| mozjs::conversions::jsstr_to_string(&mozjs::context::JSContext::from_ptr(NonNull::new_unchecked(cx)), nn));
    }
