...
pub fn install(cx: &mut mozjs::context::JSContext) {
    ...
    unsafe {
        ...
        let domain_fn = JS_NewFunction(
            cx.raw_cx(),
            Some(domain_constructor),
            0,
            0x400,
            c"Domain".as_ptr(),
        );
...
unsafe extern "C" fn domain_run(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
    ...
    let call_args = if extra_args.is_empty() {
        ...
        HandleValueArray {
            length_: extra_args.len(),
            elements_: extra_args.as_ptr(),
        }
    };
