...
fn extract_js_callback(
    ...
    unsafe {
        JS_GetProperty(cx, obj_h, c_prop.as_ptr(),
            MutableHandle::<Value> { _phantom_0: ::std::marker::PhantomData, ptr: &mut cv });
    }
    if cv.is_object() {
        let cx2 = unsafe { mozjs::context::JSContext::from_ptr(NonNull::new_unchecked(cx)) };
        rooted!(&in(cx2) let co = cv.to_object());
        if unsafe { JS_ObjectIsFunction(co.get()) } { return Some(co.get()); }
    }
    None
}
...
/// @trace REQ-BAO-API-017 [api:Bun.udpSocket]
unsafe fn invoke_js_callback(
    cx: *mut JSContext,
    cb_key: &Option<String>,
    args: &[JSVal],
) -> bool {
    ...
    if cx.is_null() { return false; }

    let Some(cb_obj) = gc_store_get(cx, key) else {
        return false;
    };
    if cb_obj.is_null() { return false; }
    ...
    if global.get().is_null() { return false; }
...
unsafe extern "C" fn bun_udp_socket(
    cx: *mut JSContext,
    argc: u32,
    vp: *mut JSVal,
) -> bool {
    ...
    if argc > 0 {
        ...
        if opts_val.is_object() {
            ...
            JS_GetProperty(cx, opts_h, c"flags".as_ptr(),
                MutableHandle::<Value> { _phantom_0: ::std::marker::PhantomData, ptr: &mut fv });
            if fv.is_int32() { flags = fv.to_int32(); }

            // connect: { hostname, port } — connected UDP mode
            let mut cv = UndefinedValue();
            JS_GetProperty(cx, opts_h, c"connect".as_ptr(),
                MutableHandle::<Value> { _phantom_0: ::std::marker::PhantomData, ptr: &mut cv });
            if cv.is_object() {
                ...
                JS_GetProperty(cx, conn_h, c"hostname".as_ptr(),
                    MutableHandle::<Value> { _phantom_0: ::std::marker::PhantomData, ptr: &mut chv });
                if chv.is_string() { connect_hostname = Some(crate::js_to_rust_string(cx, chv)); }

                let mut cpv = UndefinedValue();
                JS_GetProperty(cx, conn_h, c"port".as_ptr(),
                    MutableHandle::<Value> { _phantom_0: ::std::marker::PhantomData, ptr: &mut cpv });
                if cpv.is_int32() { connect_port = cpv.to_int32().max(0) as u16; }
                else if cpv.is_double() { connect_port = cpv.to_double().max(0.0) as u16; }
            }
    ...
    // Store callbacks
    let data_cb_key = on_data.map(|cb| { let k = gc_store_unique_key(&format!("udp_data_{}", udp_id)); gc_store_insert(cx, &k, cb); k });
    let drain_cb_key = on_drain.map(|cb| { let k = gc_store_unique_key(&format!("udp_drain_{}", udp_id)); gc_store_insert(cx, &k, cb); k });
    let close_cb_key = on_close.map(|cb| { let k = gc_store_unique_key(&format!("udp_close_{}", udp_id)); gc_store_insert(cx, &k, cb); k });
    let error_cb_key = on_error.map(|cb| { let k = gc_store_unique_key(&format!("udp_error_{}", udp_id)); gc_store_insert(cx, &k, cb); k });
    ...
    if loop_.is_null() {
        ...
        unsafe { reject_udp_promise(cx, promise, "Bun.udpSocket: event loop not available"); }
        rooted!(&in(cx_ref) let p = promise);
    ...
    if udp_socket.is_null() || err != 0 {
        ...
        unsafe { reject_udp_promise(cx, promise, &err_msg); }
        rooted!(&in(cx_ref) let p = promise);
        args.rval().set(ObjectValue(p.get()));
        return true;
    }
    ...
    if let Some(ref conn_host) = connect_hostname {
        if !conn_host.is_empty() && connect_port > 0 {
            ...
            let ret = unsafe { (*udp_socket).connect((*conn_cstr).as_cstr().as_ptr(), connect_port as u32) };
            if ret != 0 {
                unsafe { (*udp_socket).close(); }
                let _ = Box::from_raw(ud_ptr as *mut UdpUserData);
                unsafe { reject_udp_promise(cx, promise, "Bun.udpSocket: connect failed"); }
                rooted!(&in(cx_ref) let p = promise);
                args.rval().set(ObjectValue(p.get()));
                return true;
            }
    ...
    if udp_obj.get().is_null() {
        unsafe { (*udp_socket).close(); }
        let _ = Box::from_raw(ud_ptr as *mut UdpUserData);
        unsafe { reject_udp_promise(cx, promise, "Bun.udpSocket: failed to create JS object"); }
        rooted!(&in(cx_ref) let p = promise);
        args.rval().set(ObjectValue(p.get()));
        return true;
    }
    ...
    unsafe { JS_DefineProperty(cx, udp_h, c"_socketPtr".as_ptr(), sv.handle().into(), 0); }

    // Store ud_ptr for cleanup
    let ud_jsval = PrivateValue(ud_ptr as *const core::ffi::c_void);
    rooted!(&in(cx_ref) let uv = ud_jsval);
    unsafe { JS_DefineProperty(cx, udp_h, c"_udPtr".as_ptr(), uv.handle().into(), 0); }

    // Expose address info
    let exposed_port = if bound_port > 0 { bound_port } else { port as i32 };
    rooted!(&in(cx_ref) let port_v = Int32Value(exposed_port));
    unsafe { JS_DefineProperty(cx, udp_h, c"port".as_ptr(), port_v.handle().into(), JSPROP_ENUMERATE as u32); }
    ...
    unsafe {
        ...
        if !hn_str.is_null() {
            ...
            JS_DefineProperty(cx, udp_h, c"hostname".as_ptr(), hn_v.handle().into(), JSPROP_ENUMERATE as u32);
        }
    }
    ...
    unsafe extern "C" fn udp_send(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
        ...
        if let Some(ud_ref) = ud {
            if ud_ref.closed.get() {
                ...
                JS_GetPendingException(cx,
                    MutableHandle::<Value> { _phantom_0: ::std::marker::PhantomData, ptr: &mut ex });
                if !ex.is_undefined() { JS_ClearPendingException(cx); }
        ...
        let (target_port, target_addr) = if is_connected {
            ...
            let tp: u16 = if argc > 1 { (*args.get(1).ptr).to_int32().max(0) as u16 } else { 0 };
    ...
    unsafe extern "C" fn udp_send_many(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
        ...
        JS_GetProperty(cx, arr_h, c"length".as_ptr(),
            MutableHandle::<Value> { _phantom_0: ::std::marker::PhantomData, ptr: &mut arr_len_val });
        let arr_len = if arr_len_val.is_int32() { arr_len_val.to_int32().max(0) as usize } else { 0 };
        ...
        for i in 0..num_packets {
            ...
            JS_GetProperty(cx, arr_h, idx_cstr.as_ptr(),
                MutableHandle::<Value> { _phantom_0: ::std::marker::PhantomData, ptr: &mut elem });

            let data_str = if elem.is_string() { crate::js_to_rust_string(cx, elem) } else { String::new() };
            ...
            if is_connected {
                ...
                JS_GetProperty(cx, arr_h, port_cstr.as_ptr(),
                    MutableHandle::<Value> { _phantom_0: ::std::marker::PhantomData, ptr: &mut port_val });
                let pkt_port: u16 = if port_val.is_int32() { port_val.to_int32().max(0) as u16 } else { 0 };

                // Get address
                let mut addr_val = UndefinedValue();
                let addr_cstr = ZBox::from_bytes(format!("{}", data_idx + 2).as_bytes());
                JS_GetProperty(cx, arr_h, addr_cstr.as_ptr(),
                    MutableHandle::<Value> { _phantom_0: ::std::marker::PhantomData, ptr: &mut addr_val });
                let pkt_addr = if addr_val.is_string() { crate::js_to_rust_string(cx, addr_val) } else { "127.0.0.1".to_string() };
    ...
    unsafe extern "C" fn udp_close_fn(cx: *mut JSContext, _argc: u32, vp: *mut JSVal) -> bool {
        ...
        if !socket_ptr.is_null() {
            ...
            unsafe { (*socket_ptr).close(); }
            let undef = UndefinedValue();
            let undef_h = Handle::<Value> { _phantom_0: ::std::marker::PhantomData, ptr: &undef };
        ...
        if ud_val.is_double() && (ud_val.asBits_ & 0xFFFF000000000000) == 0 {
            ...
            if !ud_ptr.is_null() { drop(Box::from_raw(ud_ptr)); }
            let undef = UndefinedValue();
            let undef_h = Handle::<Value> { _phantom_0: ::std::marker::PhantomData, ptr: &undef };
    ...
    unsafe extern "C" fn udp_address(cx: *mut JSContext, _argc: u32, vp: *mut JSVal) -> bool {
        ...
        let mut buf = [0u8; 64];
        let mut len = 64i32;
        unsafe { (*socket_ptr).bound_ip(buf.as_mut_ptr(), &mut len); }
        ...
        if !ret_root.get().is_null() {
            ...
            if !js_ip.is_null() {
                ...
                JS_DefineProperty(cx, ret_root.handle().into(), c"address".as_ptr(), ip_v.handle().into(), JSPROP_ENUMERATE as u32);
            }
            let bound = unsafe { (*socket_ptr).bound_port() };
            rooted!(&in(cx_ref) let p_v = Int32Value(bound));
            JS_DefineProperty(cx, ret_root.handle().into(), c"port".as_ptr(), p_v.handle().into(), JSPROP_ENUMERATE as u32);
            ...
            if !js_fam.is_null() {
                ...
                JS_DefineProperty(cx, ret_root.handle().into(), c"family".as_ptr(), fam_v.handle().into(), JSPROP_ENUMERATE as u32);
            }
    ...
    unsafe extern "C" fn udp_set_broadcast(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
        ...
        let enabled = if argc > 0 { (*args.get(0).ptr).to_boolean() } else { false };
    ...
    #[allow(unsafe_op_in_unsafe_fn)]
    unsafe extern "C" fn udp_set_multicast_ttl(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
        let args = CallArgs::from_vp(vp, argc);
        let mut wrapped_cx = mozjs::context::JSContext::from_ptr(NonNull::new_unchecked(cx));
        let cx_ref = &mut wrapped_cx;
        rooted!(&in(cx_ref) let this_obj = args.thisv().to_object());
    ...
    #[allow(unsafe_op_in_unsafe_fn)]
    unsafe extern "C" fn udp_set_multicast_loopback(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
        let args = CallArgs::from_vp(vp, argc);
        let mut wrapped_cx = mozjs::context::JSContext::from_ptr(NonNull::new_unchecked(cx));
        let cx_ref = &mut wrapped_cx;
        rooted!(&in(cx_ref) let this_obj = args.thisv().to_object());
    ...
    #[allow(unsafe_op_in_unsafe_fn)]
    unsafe extern "C" fn udp_set_multicast_interface(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
        let args = CallArgs::from_vp(vp, argc);
        let mut wrapped_cx = mozjs::context::JSContext::from_ptr(NonNull::new_unchecked(cx));
        let cx_ref = &mut wrapped_cx;
        rooted!(&in(cx_ref) let this_obj = args.thisv().to_object());
    ...
    #[allow(unsafe_op_in_unsafe_fn)]
    unsafe extern "C" fn udp_drop_membership(cx: *mut JSContext, argc: u32, vp: *mut JSVal) -> bool {
    ...
    /// @trace REQ-BAO-API-017 [api:Bun.udpSocket]
    #[allow(unsafe_op_in_unsafe_fn)]
    unsafe fn udp_set_membership_impl(cx: *mut JSContext, argc: u32, vp: *mut JSVal, drop: bool) -> bool {
        let args = CallArgs::from_vp(vp, argc);
        let mut wrapped_cx = mozjs::context::JSContext::from_ptr(NonNull::new_unchecked(cx));
        let cx_ref = &mut wrapped_cx;
        rooted!(&in(cx_ref) let this_obj = args.thisv().to_object());
        ...
        // Optional interface
        let mut iface_storage: libc::sockaddr_storage = ::std::mem::zeroed();
        let iface_opt: Option<&libc::sockaddr_storage> = if argc > 1 && (*args.get(1).ptr).is_string() {
        ...
        if argc > 0 && (*args.get(0).ptr).is_object() {
            ...
            if let Some(ud_ref) = ud {
                if let Some(cb) = extract_js_callback(cx, sock_h, "data") {
                    if let Some(ref old_key) = ud_ref.data_cb_key { gc_store_remove(cx, old_key); }
                ...
                if let Some(cb) = extract_js_callback(cx, sock_h, "drain") {
                    if let Some(ref old_key) = ud_ref.drain_cb_key { gc_store_remove(cx, old_key); }
                ...
                if let Some(cb) = extract_js_callback(cx, sock_h, "close") {
                    if let Some(ref old_key) = ud_ref.close_cb_key { gc_store_remove(cx, old_key); }
                ...
                if let Some(cb) = extract_js_callback(cx, sock_h, "error") {
                    if let Some(ref old_key) = ud_ref.error_cb_key { gc_store_remove(cx, old_key); }
    ...
    unsafe {
        mozjs_sys::jsapi::JS_DefineFunction(cx, udp_h, c"send".as_ptr(), Some(udp_send), 3, JSPROP_ENUMERATE as u32);
        mozjs_sys::jsapi::JS_DefineFunction(cx, udp_h, c"sendMany".as_ptr(), Some(udp_send_many), 1, JSPROP_ENUMERATE as u32);
        mozjs_sys::jsapi::JS_DefineFunction(cx, udp_h, c"close".as_ptr(), Some(udp_close_fn), 0, JSPROP_ENUMERATE as u32);
        mozjs_sys::jsapi::JS_DefineFunction(cx, udp_h, c"address".as_ptr(), Some(udp_address), 0, JSPROP_ENUMERATE as u32);
        mozjs_sys::jsapi::JS_DefineFunction(cx, udp_h, c"ref".as_ptr(), Some(udp_ref), 0, JSPROP_ENUMERATE as u32);
        mozjs_sys::jsapi::JS_DefineFunction(cx, udp_h, c"unref".as_ptr(), Some(udp_unref), 0, JSPROP_ENUMERATE as u32);
        mozjs_sys::jsapi::JS_DefineFunction(cx, udp_h, c"connect".as_ptr(), Some(udp_connect), 2, JSPROP_ENUMERATE as u32);
        mozjs_sys::jsapi::JS_DefineFunction(cx, udp_h, c"disconnect".as_ptr(), Some(udp_disconnect), 0, JSPROP_ENUMERATE as u32);
        mozjs_sys::jsapi::JS_DefineFunction(cx, udp_h, c"setBroadcast".as_ptr(), Some(udp_set_broadcast), 1, JSPROP_ENUMERATE as u32);
        mozjs_sys::jsapi::JS_DefineFunction(cx, udp_h, c"setTTL".as_ptr(), Some(udp_set_ttl), 1, JSPROP_ENUMERATE as u32);
        mozjs_sys::jsapi::JS_DefineFunction(cx, udp_h, c"setMulticastTTL".as_ptr(), Some(udp_set_multicast_ttl), 1, JSPROP_ENUMERATE as u32);
        mozjs_sys::jsapi::JS_DefineFunction(cx, udp_h, c"setMulticastLoopback".as_ptr(), Some(udp_set_multicast_loopback), 1, JSPROP_ENUMERATE as u32);
        mozjs_sys::jsapi::JS_DefineFunction(cx, udp_h, c"setMulticastInterface".as_ptr(), Some(udp_set_multicast_interface), 1, JSPROP_ENUMERATE as u32);
        mozjs_sys::jsapi::JS_DefineFunction(cx, udp_h, c"addMembership".as_ptr(), Some(udp_add_membership), 2, JSPROP_ENUMERATE as u32);
        mozjs_sys::jsapi::JS_DefineFunction(cx, udp_h, c"dropMembership".as_ptr(), Some(udp_drop_membership), 2, JSPROP_ENUMERATE as u32);
        mozjs_sys::jsapi::JS_DefineFunction(cx, udp_h, c"reload".as_ptr(), Some(udp_reload), 1, JSPROP_ENUMERATE as u32);

        // closed getter property — use JS_DefineProperty1 with getter native
        mozjs_sys::jsapi::JS_DefineProperty1(
            cx, udp_h, c"closed".as_ptr(),
            Some(udp_get_closed),
            None,
...
unsafe fn get_socket_ptr(cx: *mut JSContext, obj_h: Handle<*mut JSObject>) -> *mut UdpSocket {
    let mut sv = UndefinedValue();
    JS_GetProperty(cx, obj_h, c"_socketPtr".as_ptr(),
        MutableHandle::<Value> { _phantom_0: ::std::marker::PhantomData, ptr: &mut sv });
...
/// @trace REQ-BAO-API-017 [api:Bun.udpSocket]
unsafe fn get_udp_ud<'a>(cx: *mut JSContext, obj_h: Handle<*mut JSObject>) -> Option<&'a UdpUserData> {
    let mut ud_val = UndefinedValue();
    JS_GetProperty(cx, obj_h, c"_udPtr".as_ptr(),
        MutableHandle::<Value> { _phantom_0: ::std::marker::PhantomData, ptr: &mut ud_val });
    if ud_val.is_double() && (ud_val.asBits_ & 0xFFFF000000000000) == 0 {
        let ud_ptr = ud_val.to_private() as *mut UdpUserData;
        if !ud_ptr.is_null() { Some(&*ud_ptr) } else { None }
    } else {
        None
    }
}
...
/// @trace REQ-BAO-API-017 [api:Bun.udpSocket]
unsafe fn get_udp_ud_mut<'a>(cx: *mut JSContext, obj_h: Handle<*mut JSObject>) -> Option<&'a mut UdpUserData> {
    let mut ud_val = UndefinedValue();
    JS_GetProperty(cx, obj_h, c"_udPtr".as_ptr(),
        MutableHandle::<Value> { _phantom_0: ::std::marker::PhantomData, ptr: &mut ud_val });
    if ud_val.is_double() && (ud_val.asBits_ & 0xFFFF000000000000) == 0 {
        let ud_ptr = ud_val.to_private() as *mut UdpUserData;
        if !ud_ptr.is_null() { Some(&mut *ud_ptr) } else { None }
    } else {
        None
    }
}
...
extern "C" fn udp_on_data(
    ...
    if count <= 0 || packets.is_null() || socket.is_null() { return; }
    ...
    for i in 0..count {
        ...
        let flags_val = if !flags_obj.is_null() {
            ...
            unsafe { JS_DefineProperty(cx, fo_h, c"truncated".as_ptr(), trunc_v.handle().into(), JSPROP_ENUMERATE as u32); }
            ObjectValue(flags_obj)
        } else {
            UndefinedValue()
        };

        let _ = unsafe { invoke_js_callback(cx, &ud.data_cb_key, &[data_val, port_val, addr_val, flags_val]) };
    }
}
...
extern "C" fn udp_on_recv_error(socket: *mut UdpSocket, code: ::std::ffi::c_int) {
    ...
    if !err_obj.is_null() {
        ...
        unsafe { JS_DefineProperty(cx, eo_h, c"errno".as_ptr(), errno_v.handle().into(), JSPROP_ENUMERATE as u32); }
        ...
        if !js_msg.is_null() {
            ...
            unsafe { JS_DefineProperty(cx, eo_h, c"message".as_ptr(), msg_v.handle().into(), JSPROP_ENUMERATE as u32); }
        }
        ...
        if !js_code.is_null() {
            ...
            unsafe { JS_DefineProperty(cx, eo_h, c"code".as_ptr(), code_v.handle().into(), JSPROP_ENUMERATE as u32); }
        }
...
pub unsafe fn install(
    ...
    mozjs_sys::jsapi::JS_DefineFunction(
        raw_cx, bun_h, c"udpSocket".as_ptr(), Some(bun_udp_socket), 2, JSPROP_ENUMERATE as u32,
    );
}
