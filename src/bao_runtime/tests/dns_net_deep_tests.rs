...
fn test_dns_net_deep() {
    ...
    // =============================================
    assert!(
        eval_bool(&mut ctx, "typeof require('dns') === 'object'"),
        "dns should be object"
    );

    // dns.lookup exists
    assert!(
        eval_bool(&mut ctx, "typeof require('dns').lookup === 'function'"),
        "dns.lookup should be function"
    );

    // dns.resolve exists
    assert!(
        eval_bool(&mut ctx, "typeof require('dns').resolve === 'function'"),
        "dns.resolve should be function"
    );

    // dns.resolve4 exists
    assert!(
        eval_bool(&mut ctx, "typeof require('dns').resolve4 === 'function'"),
        "dns.resolve4 should be function"
    );

    // dns.resolve6 exists
    assert!(
        eval_bool(&mut ctx, "typeof require('dns').resolve6 === 'function'"),
        "dns.resolve6 should be function"
    );

    // dns.reverse exists
    assert!(
        eval_bool(&mut ctx, "typeof require('dns').reverse === 'function'"),
        "dns.reverse should be function"
    );

    // dns.Resolver constructor
    assert!(
        eval_bool(&mut ctx, "typeof require('dns').Resolver === 'function'"),
    ...
    assert!(
        lookup_result.contains("127.0.0.1")
            || lookup_result.contains("::1")
            || lookup_result.contains("lookup_err")
            || lookup_result.contains("no_result"),
        "dns.lookup should resolve localhost, got: {}",
        lookup_result
    );

    // dns.lookup returns family
    let family_result = eval_string(
        &mut ctx,
        r#"
        try {
            var dns = require('dns');
    ...
    assert!(
        family_result.contains("4")
            || family_result.contains("6")
            || family_result.contains("no_family"),
        "dns.lookup should return family, got: {}",
        family_result
    );

    // dns.resolve4 returns array for localhost
    let resolve4_result = eval_string(
        &mut ctx,
        r#"
        try {
            var dns = require('dns');
    ...
    assert!(
        resolve4_result.contains("array_ok") || resolve4_result.contains("resolve4_err"),
        "dns.resolve4 should return array, got: {}",
        resolve4_result
    );
    ...
    // =============================================
    assert!(
        eval_bool(&mut ctx, "typeof require('net') === 'object'"),
        "net should be object"
    );
    ...
    // net.Socket
    assert!(
        eval_bool(&mut ctx, "typeof require('net').Socket === 'function'"),
        "net.Socket should be function"
    );

    // net.Server
    assert!(
        eval_bool(&mut ctx, "typeof require('net').Server === 'function'"),
        "net.Server should be function"
    );

    // net.connect
    assert!(
        eval_bool(&mut ctx, "typeof require('net').connect === 'function'"),
    ...
    assert!(
        eval_bool(
            &mut ctx,
            "typeof require('net').createConnection === 'function'"
        ),
    ...
    assert_eq!(
        is_ip, 4.0,
        "isIP('127.0.0.1') should return 4, got: {}",
        is_ip
    );

    // net.isIPv4
    assert!(
        eval_bool(&mut ctx, "require('net').isIPv4('192.168.1.1')"),
        "isIPv4('192.168.1.1') should return true"
    );

    // net.isIPv4 invalid
    assert!(
        !eval_bool(&mut ctx, "require('net').isIPv4('not-an-ip')"),
    ...
    assert!(
        addr.contains("addr_ok"),
        "server.address() should return object, got: {}",
        addr
    );
