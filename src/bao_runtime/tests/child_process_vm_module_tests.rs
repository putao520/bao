...
fn test_child_process_vm_module_zlib_deep() {
    ...
    assert!(
        cp.contains("execFile"),
        "child_process should have execFile"
    );
    assert!(
        cp.contains("execSync"),
        "child_process should have execSync"
    );
    ...
    assert!(
        exec_output.contains("hello_world"),
        "execSync should return output, got: {}",
        exec_output
    );
    ...
    assert!(
        exec_file_output.contains("file_test"),
        "execFileSync should return output, got: {}",
        exec_file_output
    );
    ...
    // =============================================
    assert!(
        eval_bool(&mut ctx, "typeof require('vm') === 'object'"),
        "vm should be object"
    );

    // vm.runInThisContext — evaluates in current scope, sets side effects
    let vm_result = eval_number(
        &mut ctx,
        r#"
        var vm = require('vm');
    ...
    assert_eq!(
        vm_result, 5.0,
        "vm.runInThisContext should evaluate and set global"
    );

    // vm.runInNewContext — evaluates with sandbox, returns undefined (implementation)
    let vm_ctx_result = eval_string(
        &mut ctx,
        r#"
        var vm = require('vm');
    ...
    assert!(
        vm_ctx_result.contains("sandbox_ok") || vm_ctx_result.contains("undefined_return"),
        "vm.runInNewContext should execute in sandbox context, got: {}",
        vm_ctx_result
    );
    ...
    assert_eq!(
        script_result, 200.0,
        "vm.Script.runInThisContext should execute and set global"
    );
    ...
    // =============================================
    assert!(
        eval_bool(&mut ctx, "typeof require('module') === 'object'"),
    ...
    assert!(
        !resolved.is_empty(),
        "module._resolveFilename should return path, got: {}",
        resolved
    );
    ...
    assert!(
        builtins.contains("fs"),
        "builtinModules should contain fs, got: {}",
        builtins
    );
    ...
    assert!(
        zlib_rt.contains("Hello") || zlib_rt.contains("buffer_obj"),
        "deflate+inflate roundtrip should work, got: {}",
        zlib_rt
    );

    // zlib.gzipSync + gunzipSync roundtrip
    let gzip_rt = eval_string(
        &mut ctx,
        r#"
        var zlib = require('zlib');
    ...
    assert!(
        gzip_rt.contains("Gzip") || gzip_rt.contains("buffer_obj"),
        "gzip+gunzip roundtrip should work, got: {}",
        gzip_rt
    );

    // zlib.deflateRawSync + inflateRawSync roundtrip
    let raw_rt = eval_string(
        &mut ctx,
        r#"
        var zlib = require('zlib');
    ...
    assert!(
        raw_rt.contains("Raw deflate") || raw_rt.contains("buffer_obj"),
        "deflateRaw+inflateRaw roundtrip should work, got: {}",
        raw_rt
    );

    // zlib functions are distinct
    let zlib_fns = eval_string(
        &mut ctx,
        r#"
        var zlib = require('zlib');
    ...
    assert_eq!(
        zlib_fns, "deflateSync,inflateSync,deflateRawSync,inflateRawSync,gzipSync,gunzipSync",
        "zlib should have all 6 sync functions"
    );

    // zlib roundtrip preserves unicode
    let unicode_rt = eval_string(
        &mut ctx,
        r#"
        var zlib = require('zlib');
    ...
    assert!(
        unicode_rt.contains("Unicode") || unicode_rt.contains("buffer_obj"),
        "gzip should preserve unicode, got: {}",
        unicode_rt
    );
    ...
    assert_eq!(
        large_rt, 10000.0,
        "gzip should preserve large content length, got: {}",
        large_rt
    );
