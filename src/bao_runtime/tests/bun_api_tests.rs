...
fn test_bun_api_all() {
    ...
    assert!(
        ...
        "Bao should be same object as Bun"
    );
    assert!(
        eval_bool(&mut ctx, "Bun.env === Bao.env"),
        "Bao.env should be same as Bun.env"
    );
    assert!(
        eval_bool(&mut ctx, "Bun.version === Bao.version"),
        "Bao.version should equal Bun.version"
    );
    assert!(
        eval_bool(
            &mut ctx,
    ...
    assert!(
        !eval_string(&mut ctx, "Bun.version").is_empty(),
        "Bun.version not empty"
    );
    assert!(
        eval_bool(
            &mut ctx,
            r#"typeof Bun.env.PATH === "string" && Bun.env.PATH.length > 0"#
        ),
        "Bun.env.PATH"
    );
    assert!(
        eval_bool(&mut ctx, "Array.isArray(Bun.argv) && Bun.argv.length > 0"),
        "Bun.argv"
    );
    assert!(!eval_string(&mut ctx, "Bun.cwd()").is_empty(), "Bun.cwd()");
    assert!(eval_ok(&mut ctx, "Bun.gc()"), "Bun.gc()");
    assert!(
        !eval_string(&mut ctx, "Bun.revision").is_empty(),
        "Bun.revision"
    );
    assert_eq!(
    ...
    // --- Bun functions exist ---
    assert!(
        eval_bool(&mut ctx, "typeof Bun.exit === 'function'"),
        "Bun.exit"
    );
    assert!(
        eval_bool(&mut ctx, "typeof Bun.sleep === 'function'"),
        "Bun.sleep"
    );
    assert!(
        eval_bool(&mut ctx, "typeof Bun.sleepSync === 'function'"),
        "Bun.sleepSync"
    );
    assert!(
        eval_bool(&mut ctx, "typeof Bun.spawn === 'function'"),
        "Bun.spawn"
    );
    assert!(
        eval_bool(&mut ctx, "typeof Bun.build === 'function'"),
        "Bun.build"
    );
    assert!(
        eval_bool(&mut ctx, "typeof Bun.resolve === 'function'"),
        "Bun.resolve"
    );
    assert!(
        eval_bool(&mut ctx, "typeof Bun.test === 'function'"),
        "Bun.test"
    );
    assert!(
        eval_bool(&mut ctx, "typeof Bun.testRun === 'function'"),
        "Bun.testRun"
    );
    assert!(
        eval_bool(&mut ctx, "Bun.read === Bun.readFile"),
        "Bun.read alias"
    );
    ...
    assert!(
        eval_bool(
            &mut ctx,
            r#"typeof Bun.which("ls") === "string" || Bun.which("ls") === null"#
        ),
        "Bun.which"
    );
    ...
    assert!(
        eval_bool(
            &mut ctx,
            &format!(r#"typeof Bun.file("{}").path === "string""#, path)
        ),
        "Bun.file path"
    );
    assert!(
    ...
    assert!(
        eval_ok(
            &mut ctx,
            &format!(r#"Bun.write("{}", "hello world")"#, path2)
        ),
        "Bun.write ok"
    );
    assert_eq!(
        ::std::fs::read_to_string(&tmp2).unwrap(),
        "hello world",
        "Bun.write content"
    );
    ...
    // --- WebSocket ---
    assert!(
        eval_bool(&mut ctx, "typeof WebSocket === 'function'"),
        "WebSocket constructor"
    );

    // --- process global ---
    assert!(
        !eval_string(&mut ctx, "process.arch").is_empty(),
        "process.arch"
    );
    assert!(
        !eval_string(&mut ctx, "process.platform").is_empty(),
        "process.platform"
    );
    assert!(
        eval_string(&mut ctx, "process.version").starts_with('v'),
        "process.version"
    );
    assert!(
        eval_bool(
    ...
    assert!(
        eval_bool(&mut ctx, r#"typeof process.env.PATH === "string""#),
        "process.env.PATH"
    );
    assert!(
        eval_bool(&mut ctx, "typeof process.cwd() === 'string'"),
        "process.cwd()"
    );
    assert!(
        eval_bool(
    ...
    assert_eq!(
        eval_string(&mut ctx, "process.title"),
        "bao",
        "process.title"
    );
    assert!(
    ...
    assert!(
        eval_bool(
            &mut ctx,
            "typeof process.uptime() === 'number' && process.uptime() >= 0"
        ),
        "process.uptime"
    );
    assert!(
