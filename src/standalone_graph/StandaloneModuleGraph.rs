...
impl StandaloneModuleGraph {
    ...
    pub fn hint_source_pages_dont_need() {
        let Some(graph) = Self::get() else {
            return;
        };
        // SAFETY: the graph is the process-global INSTANCE singleton (set once
        // at startup, never freed; see `unsafe impl Sync for Instance`); this
        // read-only walk of the file table is the resolver-facing read path
        // that impl was declared for.
