...
| 当前手写模块 | 迁移目标 | 优先级 | 状态 |
|-------------|---------|--------|------|
| `bao_engine/module_loader.rs` 中的 `resolve_specifier` | 桥接 `bun_resolver` | P0 | 待迁移（需 Resolver 依赖评估） |
| `bao_runtime/require.rs` 中的 `resolve_specifier` | 桥接 `bun_resolver` | P0 | 待迁移（需 Resolver 依赖评估） |
