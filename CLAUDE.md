...
systemd timer 每日 06:07±10min 运行 `.claude/skills/daily-ops/`(上游轻量同步+issue 分诊,详见该 skill)。交互会话在此窗口派工前先查 `systemctl --user status bao-daily-ops`。上游同步基线 SSOT = `.claude/upstream-baseline.json`。
