...
systemd timer 每日 06:07±10min 运行 `.claude/skills/daily-ops/`(全自主:上游任意窗口吸收含 BCE patch 重放 + issue 根治 + 波末验收 + 发布闭包,2026-08-24 用户裁决扩权;mozjs 跨版本升级除外,详见该 skill)。交互会话在此窗口派工前先查 `systemctl --user status bao-daily-ops`。上游同步基线 SSOT = `.claude/upstream-baseline.json`。
