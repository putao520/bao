...
**Bun 适配 Servo。遇到冲突改 Bun 不改 Servo。Servo 是上游，Bun 是下游。**
...
- **Servo 代码是上游真源，禁止修改 servo 上游代码。Bao 层（bao_engine/bao_browser/bao_cdp/bao_stealth）适配 servo 的接口和数据模型**
- **JSContext 融合：始终寄生 servo 的 JSContext，不存在独立 JSContext。所有模式（CLI/browser/CDP）共享 servo 的唯一 JSContext，DOM↔Node.js 对象原生互操作**
- **Bun 适配 Servo：servo 拥有 JSContext 生命周期，bao_engine 通过 RustRuntime::get() 获取同一指针，不创建独立 Runtime**
...
| 当前手写模块 | 迁移目标 | 优先级 | 状态 |
|-------------|---------|--------|------|
| `bao_engine/module_loader.rs` 中的 `resolve_specifier` | 桥接 `bun_resolver` | P0 | ✅ 已用 resolver_bridge + bun_resolver::Resolver |
| `bao_runtime/require.rs` 中的 `resolve_specifier` | 桥接 `bun_resolver` | P0 | ✅ 已用 bao_engine::try_external_resolve |
| `bao_runtime/node_http.rs` | 桥接 `bun_http` + `bun_uws` | P0 | ✅ 已用 bun_uws::App 替代 |
| `bao_runtime/timers.rs` (TimerHeap + sleep 轮询) | 桥接 `bun_event_loop` uSockets timer | P1 | ✅ 已用 bun_io::heap::Intrusive + EventLoopTimer |
| `bao_runtime/node_fs.rs` 中的 `base64_encode` | 替换为 `bun_base64` crate | P1 | ✅ 已用 bun_base64::encode_alloc |
| `bao_runtime/fetch_api.rs` 中的 `do_fetch` | 桥接 `bun_http` + stealth_http | P2 | ✅ 已用 stealth_http_request 替代 |
...
`mozjs/mozjs-sys/mozjs/mozglue/misc/Mutex_posix.cpp` 中的 `MutexImpl::~MutexImpl` 已 patch：
- 原始代码：`pthread_mutex_destroy` 返回非零时调用 `MOZ_CRASH`（SIGSEGV）
...
- EBUSY patch 生效后，`cargo test` 默认多线程模式零 SIGSEGV（546+ tests passed）
- **不需要** `--test-threads=1`
- 如果 SIGSEGV 复现，第一步检查 rlib 是否包含旧代码（`nm libmozjs_sys-*.rlib | grep MutexImplD1` 查看符号）
