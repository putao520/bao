...
**Next single action: B1 slice 2 is SPLIT by an explicit user-ruling gate (2026-09-17).**
- **Closed 2026-09-17 (documentation-only, zero behavior delta)**: row 18 `PROCESS_MEMORY_BRIDGE` — generation-token teardown was already the enforced contract shape (contract doc added, census "skips clearing" reframed as the correct guarded skip); row 23 TLS `DRIVER` — documented-accept as a process-level infrastructure thread (no per-runtime state; same family as HTTPThread / bun_dns resolver; entries owned by JS-object lifecycles). Row 26 was already PUBLIC-PROCESS-SEMANTICS (no action); rows 16/20 were previously consumed (`dad8135d` / R53-A).
- **PENDING USER RULING — rows 22/24/25** (`UDP_REGISTRY` / `WORKER_REGISTRY` / `CP_ASYNC_STATES`+`CP_IPC_CHANNELS`): runtime-drop semantics for live unclosed resources is embedder-visible behavior, not an engineering detail (0A gate). Asked 2026-09-17 (user AFK after 60s; re-ask). Options presented: (A) full termination — runtime owns everything, drop ≈ miniature process.exit (worker terminate / socket close / child kill) [recommended]; (B) exact Node process-exit mirror (child orphans, workers abort, fds to OS); (C) registry-entry removal only, resources survive; (D) documented-accept status quo. On ruling: implement per-runtime token cleanup (row-18 token pattern generalized) across the three registries.
- Weak rows 27-31 fold into whichever shape the ruling picks (27 `TOP_LEVEL_DIR`, 29 argv/streams, 30 engine hooks follow the ownership verdict; 28 is CLI-product-exempt, 31 is a C-ABI mirror — both documented-accept).
